  ...
  return (
    <div className="relative">
      <button
        ...
        className="relative w-8 h-8 rounded-lg hover:bg-bg flex items-center justify-center transition-colors"
      >
        {unreadCount > 0 ? (
            ...
            <div className="flex-1 overflow-y-auto">
              {alerts.length === 0 ? (
                ...
                alerts.slice(0, 30).map((a) => (
                  <div
                    key={a.id}
                    className={`px-3 py-2.5 border-b border-border/50 hover:bg-bg/50 transition-colors ${!a.read ? "bg-primary/5" : ""}`}
                  >
                    <div className="flex items-start gap-2">
                      <span
                        className={`text-[9px] font-bold px-1.5 py-0.5 rounded shrink-0 ${SEVERITY_COLORS[a.severity] ?? SEVERITY_COLORS.info}`}
                      >
...
export function AlertManager() {
  ...
  const addRule = async () => {
    ...
    await createAlertRule(user.id, {
      ...
      severity,
      message: messages[type] ?? `${symbol} ${type}`,
    });
    setShowForm(false);
    setThreshold("");
    loadRules();
    ...
    <div className="bg-surface border border-border rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-xs font-semibold flex items-center gap-1.5">
          <Zap className="w-3.5 h-3.5 text-primary" /> Alert Rules
        </h3>
        <button
      ...
      {showForm && (
        <div className="mb-3 p-3 rounded-lg bg-bg border border-border space-y-2">
          ...
          <button
            onClick={addRule}
            className="w-full py-1.5 rounded bg-primary text-black text-xs font-medium"
          >
            Create Rule
          </button>
        </div>
          ...
          {rules.map((r) => (
            ...
            >
              <button
                ...
                className={`w-8 h-4 rounded-full transition-colors ${r.enabled ? "bg-primary" : "bg-muted/30"}`}
              >
                <span
                  className={`block w-3 h-3 rounded-full bg-white transition-transform ${r.enabled ? "translate-x-4" : "translate-x-0.5"}`}
                />
              </button>
              <span className="flex-1 truncate">{r.message}</span>
              <span
                className={`text-[9px] px-1 py-0.5 rounded ${SEVERITY_COLORS[r.severity] ?? ""}`}
              >
                {r.severity}
              </span>
              <button onClick={() => deleteRule(r.id)} className="text-muted hover:text-danger">
