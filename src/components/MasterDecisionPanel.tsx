...
const ACTION_STYLE: Record<
  DecisionAction,
  { cls: string; label: string; Icon: typeof TrendingUp }
> = {
  ...
  return (
    <div className="rounded-xl border border-border bg-surface p-4 space-y-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <div
          className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-xs font-semibold ${style.cls}`}
        >
      ...
      <div className="rounded-lg border border-border p-3">
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span
            className={`text-xs font-semibold ${(quality?.score ?? 0) >= 80 ? "text-success" : (quality?.score ?? 0) >= 50 ? "text-warning" : "text-danger"}`}
          >
            {quality?.score ?? 0}/100
          </span>
        </div>
        {quality && quality.issues.length === 0 ? (
          <p className="text-[11px] text-muted">
            No anomalies detected · {quality.bars} bars ·{" "}
            {quality.fresh ? "fresh feed" : "stale feed"}
          </p>
        ) : (
          <ul className="space-y-1">
            {(quality?.issues ?? []).slice(0, 5).map((i) => (
              <li key={i.code} className="text-[11px] text-muted flex items-start gap-1.5">
                <AlertTriangle
                  className={`w-3 h-3 mt-0.5 shrink-0 ${i.severity === "critical" ? "text-danger" : "text-warning"}`}
                />
                {i.detail}
              </li>
            ))}
      ...
      <div className="rounded-lg border border-border p-3">
        <div className="flex items-center gap-1.5 mb-1.5">
          ...
          <span className="text-xs font-medium">
            Risk Gate — {gate?.approved ? "Approved" : "Not approved"}
          </span>
        </div>
        <p className="text-[11px] text-muted">
          {gate?.reason ?? "Risk gate did not run: pipeline halted earlier."}
        </p>
      </div>
          ...
          <ul className="space-y-1 mt-1.5">
            {contradictions.map((c) => (
              <li key={c.code} className="text-[11px] text-muted flex items-start gap-1.5">
                <span
                  className={`mt-1 w-1.5 h-1.5 rounded-full shrink-0 ${c.severity === "high" ? "bg-danger" : c.severity === "medium" ? "bg-warning" : "bg-muted"}`}
                />
                {c.detail}
              </li>
            ))}
        ...
        <ol className="space-y-2">
          {groupByLayer(decision.pipeline).map((group, gIdx) => (
            <li key={`${group.layer}-${gIdx}`}>
              <p className="text-[9px] uppercase tracking-[0.12em] text-muted mb-1">
                {LAYER_LABEL[group.layer]}
              </p>
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-baseline gap-1.5 flex-wrap">
                        ...
                        <span className="text-[9px] font-mono text-muted">
                          v{run.result.engine_version}
                        </span>
                      </div>
                      <p className="text-[10px] text-muted mt-0.5 break-words leading-snug">
                        {run.verdict}
                      </p>
                    </div>
                    <div className="text-right shrink-0 leading-tight">
                      <p
                        className={`text-[11px] font-semibold tabular-nums ${STATUS_TEXT[run.result.status]}`}
                      >
                        {(run.result.confidence * 100).toFixed(0)}%
                      </p>
                      <p className="text-[9px] text-muted font-mono tabular-nums">
      ...
      <div className="flex flex-wrap gap-1.5">
        {Object.entries(decision.engineVersions).map(([name, version]) => (
          <span
            key={name}
            className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-bg border border-border text-muted"
          >
            {name}@{version}
          </span>
        ))}
      ...
      <p className="text-[10px] text-muted">
        Paper trading only — the risk gate is authoritative and cannot be bypassed.
      </p>
    </div>
  );
}
