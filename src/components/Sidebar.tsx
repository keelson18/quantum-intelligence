...
const NAV_ITEMS: NavItem[] = [
  { to: "/", label: "Dashboard", icon: LayoutDashboard },
  {
    ...
    icon: Terminal,
    roles: ["trader", "analyst", "admin", "super_admin"],
  },
...
export default function Sidebar() {
  ...
  return (
    ...
    >
      {/* Logo / brand */}
      <div
        className={`h-14 flex items-center border-b border-border shrink-0 ${collapsed ? "justify-center" : "px-4"}`}
      >
      ...
      {!collapsed && (
        <div className="px-4 py-2 border-t border-border shrink-0">
          <div className="flex items-center gap-2">
            ...
            <button
              className="ml-auto relative text-muted hover:text-text transition-colors"
              title="Alerts"
            >
      ...
      <div className="border-t border-border p-2 space-y-0.5 shrink-0">
        {collapsed && (
          <button
            className="relative flex items-center justify-center w-full py-2 text-muted hover:text-text transition-colors"
            title={`Alerts (${unreadCount})`}
          >
            <Bell className="w-4 h-4" />
            {unreadCount > 0 && (
