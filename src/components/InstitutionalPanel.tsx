    ...
    return (
      <div className="bg-surface border border-border rounded-xl p-4">
        ...
        <div className="py-6 text-center text-muted text-xs">
          Insufficient data for institutional analysis
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-3">
      ...
      <div className="bg-surface border border-border rounded-xl p-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="text-center">
            ...
            <div
              className={`text-2xl font-bold rounded-lg py-1 ${GRADE_COLORS[tradeGrade] ?? GRADE_COLORS["C"]}`}
            >
              {tradeGrade}
            </div>
          ...
          <>
            <div className="flex gap-1 mb-3 flex-wrap">
              {multiTimeframe.timeframes.map((tf) => (
                <div
                  key={tf.timeframe}
                  className="flex-1 min-w-[100px] bg-bg/50 rounded-lg p-2 border border-border/50"
                >
                  <div className="text-[10px] text-muted font-medium">
                    {MTF_LABELS[tf.timeframe]}
                  </div>
                  <div
                    className={`text-xs font-semibold flex items-center gap-0.5 ${tf.trend === "bullish" ? "text-success" : tf.trend === "bearish" ? "text-danger" : "text-muted"}`}
                  >
                    {tf.trend === "bullish" ? (
                  ...
                  <div className="text-[10px] text-muted mt-0.5">
                    {granularRegimeLabel(tf.granularRegime)}
                  </div>
            ...
            <div className="flex items-center gap-2 text-xs">
              ...
              <div className="flex-1 h-2 rounded-full bg-bg overflow-hidden">
                <div
                  className={`h-full rounded-full ${multiTimeframe.alignedDirection === "buy" ? "bg-success" : multiTimeframe.alignedDirection === "sell" ? "bg-danger" : "bg-muted"}`}
                  style={{ width: `${multiTimeframe.alignmentScore * 100}%` }}
                />
              </div>
              <span className="tabular-nums font-medium">
                {(multiTimeframe.alignmentScore * 100).toFixed(0)}%
              </span>
            </div>
          </>
        ) : (
      ...
      <div className="bg-surface border border-border rounded-xl p-4">
        ...
        <div className="mt-3 pt-2 border-t border-border flex items-center justify-between text-xs">
          ...
          <span
            className={`font-bold text-sm ${confluence.total >= 80 ? "text-success" : confluence.total >= 65 ? "text-primary" : confluence.total >= 50 ? "text-warning" : "text-danger"}`}
          >
      ...
      <div className="bg-surface border border-border rounded-xl p-4">
            ...
            <div className="space-y-0.5">
              {liquidity.buySideLiquidity.slice(0, 3).map((l, i) => (
                <div key={i} className="flex items-center justify-between text-xs">
                  ...
                  <span
                    className={`text-[10px] ${l.strength === "high" ? "text-success" : l.strength === "medium" ? "text-warning" : "text-muted"}`}
                  >
                    {l.strength}
                  </span>
                </div>
              ))}
              {liquidity.buySideLiquidity.length === 0 && (
            ...
            <div className="space-y-0.5">
              {liquidity.sellSideLiquidity.slice(0, 3).map((l, i) => (
                <div key={i} className="flex items-center justify-between text-xs">
                  <span className="tabular-nums">${l.level.toFixed(2)}</span>
                  <span
                    className={`text-[10px] ${l.strength === "high" ? "text-danger" : l.strength === "medium" ? "text-warning" : "text-muted"}`}
                  >
                    {l.strength}
                  </span>
                </div>
              ))}
              {liquidity.sellSideLiquidity.length === 0 && (
                <div className="text-xs text-muted">None detected</div>
              )}
            </div>
          </div>
        ...
        {liquidity.sweepDetected && (
          <div
            className={`text-xs px-2 py-1.5 rounded-lg ${liquidity.sweepDirection === "buy_side" ? "bg-danger/10 text-danger" : "bg-success/10 text-success"}`}
          >
            <AlertTriangle className="w-3 h-3 inline mr-1" />
            {liquidity.sweepDirection === "buy_side" ? "Buy-side" : "Sell-side"} sweep detected (
            {(liquidity.sweepProbability * 100).toFixed(0)}% probability)
          </div>
        )}
        ...
        <div className="bg-surface border border-border rounded-xl p-4">
          ...
          <div className="space-y-1 text-xs">
            <div className="flex justify-between">
              <span className="text-muted">Rejections tracked</span>
              <span className="tabular-nums">{marketMemory.rejections.length}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted">Failed breakouts</span>
              <span className="tabular-nums">{marketMemory.failedBreakouts.length}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted">Hist. support levels</span>
              <span className="tabular-nums">{marketMemory.historicalSupport.length}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted">Hist. resistance levels</span>
              <span className="tabular-nums">{marketMemory.historicalResistance.length}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted">Reaction score</span>
              <span
                className={`tabular-nums font-medium ${marketMemory.reactionScore > 0.5 ? "text-success" : marketMemory.reactionScore > 0.2 ? "text-warning" : "text-muted"}`}
              >
                {(marketMemory.reactionScore * 100).toFixed(0)}%
              </span>
            </div>
          </div>
        ...
        <div className="bg-surface border border-border rounded-xl p-4">
          ...
          <div className="space-y-1 text-xs">
            {marketContext.nearestResistance && (
              <div className="flex justify-between">
                <span className="text-muted">Nearest resistance</span>
                <span className="tabular-nums text-danger">
                  {marketContext.nearestResistance.distancePct.toFixed(1)}%
                </span>
              </div>
            )}
            {marketContext.nearestSupport && (
              <div className="flex justify-between">
                <span className="text-muted">Nearest support</span>
                <span className="tabular-nums text-success">
                  {marketContext.nearestSupport.distancePct.toFixed(1)}%
                </span>
              </div>
            )}
            <div className="flex justify-between">
              ...
              <span
                className={
                  marketContext.volatilityCondition === "high"
                    ? "text-danger"
                    : marketContext.volatilityCondition === "low"
                      ? "text-muted"
                      : "text-warning"
                }
              >
                {marketContext.volatilityCondition}
              </span>
            </div>
            <div className="flex justify-between">
              ...
              <span
                className={
                  marketContext.liquidityCondition === "high"
                    ? "text-success"
                    : marketContext.liquidityCondition === "low"
                      ? "text-muted"
                      : "text-warning"
                }
              >
                {marketContext.liquidityCondition}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted">Trend strength</span>
              <span className="tabular-nums">
                {(marketContext.trendStrength * 100).toFixed(0)}%
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted">Context penalty</span>
              <span className="tabular-nums text-warning">
                -{(marketContext.contextPenalty * 100).toFixed(0)}%
              </span>
            </div>
          </div>
      ...
      {portfolioIntelligence && (
        <div className="bg-surface border border-border rounded-xl p-4">
          ...
          <div className="grid grid-cols-3 gap-2 mb-2">
            <div className="bg-bg/50 rounded p-2">
              ...
              <div className="text-xs font-medium tabular-nums">
                {(portfolioIntelligence.totalExposure * 100).toFixed(0)}%
              </div>
            </div>
            <div className="bg-bg/50 rounded p-2">
              <div className="text-[10px] text-muted">Correlation</div>
              <div
                className={`text-xs font-medium ${portfolioIntelligence.correlationRisk === "high" ? "text-danger" : portfolioIntelligence.correlationRisk === "medium" ? "text-warning" : "text-success"}`}
              >
                {portfolioIntelligence.correlationRisk}
              </div>
            </div>
            <div className="bg-bg/50 rounded p-2">
              <div className="text-[10px] text-muted">Concentration</div>
              <div
                className={`text-xs font-medium ${portfolioIntelligence.concentrationRisk === "high" ? "text-danger" : portfolioIntelligence.concentrationRisk === "medium" ? "text-warning" : "text-success"}`}
              >
                {portfolioIntelligence.concentrationRisk}
              </div>
            </div>
          </div>
          ...
          <div className="text-[10px] text-muted mt-1">
            Position multiplier:{" "}
            {(portfolioIntelligence.suggestedPositionMultiplier * 100).toFixed(0)}%
          </div>
        </div>
      )}
...
function ConfluenceBar({ label, value, max }: { label: string; value: number; max: number }) {
  ...
  const color =
    pct >= 75 ? "bg-success" : pct >= 50 ? "bg-primary" : pct >= 25 ? "bg-warning" : "bg-danger";
  return (
    <div className="flex items-center gap-2 text-xs">
      ...
      <span className="w-8 text-right tabular-nums font-medium">
        {value}/{max}
      </span>
    </div>
  );
