...
export default function ModelMetrics({ metrics }: Props) {
  ...
  useEffect(() => {
    ...
    lossSeriesRef.current = {
      train: lossChartRef.current.addLineSeries({
        color: "#10a37f",
        lineWidth: 2,
      ...
      val: lossChartRef.current.addLineSeries({
        color: "#ef4444",
        lineWidth: 2,
        priceLineVisible: false,
        lastValueVisible: true,
    ...
    accSeriesRef.current = {
      train: accChartRef.current.addLineSeries({
        color: "#10a37f",
        lineWidth: 2,
        priceLineVisible: false,
        lastValueVisible: true,
      ...
      val: accChartRef.current.addLineSeries({
        color: "#ef4444",
        lineWidth: 2,
        priceLineVisible: false,
        lastValueVisible: true,
    ...
    const handleResize = () => {
      if (lossChartRef.current && lossContainerRef.current)
        lossChartRef.current.applyOptions({ width: lossContainerRef.current.clientWidth });
      if (accChartRef.current && accContainerRef.current)
        accChartRef.current.applyOptions({ width: accContainerRef.current.clientWidth });
    };
  ...
  useEffect(() => {
    ...
    const accData = metrics
      ...
      .map((m) => ({ time: m.epoch as Time, value: m.accuracy! }));
    const valAccData = metrics
      .filter((m) => m.valAccuracy != null)
      .map((m) => ({ time: m.epoch as Time, value: m.valAccuracy! }));
