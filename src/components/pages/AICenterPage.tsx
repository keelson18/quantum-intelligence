...
export default function AICenterPage() {
  ...
  }, [tab, loadModels, loadPredictions]);
    ...
    <div className="px-4 lg:px-6 py-4">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted mt-0.5">
            Internal deterministic decision engine — signals, models & predictions
          </p>
        </div>
      </div>
          ...
          <div className="space-y-2">
            {loading && signals.length === 0 ? (
              ...
              <div className="py-12 text-center text-muted text-sm">
                No active trade signals. Try a different timeframe.
              </div>
            ) : (
              signals.map((s) => (
                <div
                ...
                >
                  ...
                  >
                    <span
                      className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${s.side === "buy" ? "bg-success/15" : "bg-danger/15"}`}
                    >
                      {s.side === "buy" ? (
                    ...
                    <div className="text-right">
                      <div
                        className={`text-sm font-semibold tabular-nums ${s.score >= 0 ? "text-success" : "text-danger"}`}
                      >
                        {s.score.toFixed(3)}
                      </div>
                      <div className="text-xs text-muted tabular-nums">
                  ...
                  {expanded === s.symbol && (
                    <div className="px-4 pb-3 border-t border-border/50 pt-3">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
                        ...
                        <MiniMetric
                          label="Stop Loss"
                          value={`$${s.stopLoss.toFixed(2)}`}
                          color="text-danger"
                        />
                        <MiniMetric
                          label="Take Profit"
                          value={`$${s.takeProfit.toFixed(2)}`}
                          color="text-success"
                        />
                        <MiniMetric
                          label="Position Value"
                          value={`$${s.positionValue.toFixed(0)}`}
                        />
                      </div>
                      <div className="text-xs font-medium mb-2 flex items-center gap-1">
                        <Zap className="w-3 h-3 text-primary" /> Contributors
                      </div>
                      <div className="space-y-1">
                        {s.contributors.map((c, i) => (
                          <div key={i} className="flex items-center gap-2 text-xs">
                            <span
                              className={`w-1.5 h-1.5 rounded-full ${c.side === "buy" ? "bg-success" : c.side === "sell" ? "bg-danger" : "bg-muted"}`}
                            />
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-semibold flex items-center gap-1.5">
              <Cpu className="w-3.5 h-3.5 text-primary" /> Trained Models
            </h3>
            <button
              onClick={loadModels}
              ...
              {models.map((m) => (
                <div key={m.id} className="bg-surface border border-border rounded-xl p-4">
                  ...
                  <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
                    <MiniMetric
                      label="Accuracy"
                      value={m.accuracy ? `${(m.accuracy * 100).toFixed(1)}%` : "—"}
                    />
                    <MiniMetric
                      label="Precision"
                      value={m.precision ? `${(m.precision * 100).toFixed(1)}%` : "—"}
                    />
                    <MiniMetric
                      label="Recall"
                      value={m.recall ? `${(m.recall * 100).toFixed(1)}%` : "—"}
                    />
                    <MiniMetric
                      label="Samples"
                      value={m.training_samples?.toLocaleString() ?? "—"}
                    />
                    <MiniMetric label="Epochs" value={m.epochs?.toString() ?? "—"} />
                    <MiniMetric
                      label="Features"
                      value={m.features ? `${m.features.length}` : "—"}
                    />
                  </div>
                  {m.features && m.features.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {m.features.slice(0, 8).map((f, i) => (
                        <span
                          key={i}
                          className="text-[10px] px-1.5 py-0.5 rounded bg-bg text-muted"
                        >
                          {f}
                        </span>
                      ))}
      ...
      {tab === "predictions" && (
        <div>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-semibold flex items-center gap-1.5">
              <Target className="w-3.5 h-3.5 text-primary" /> Prediction History
            </h3>
            <button
          ...
          {predictions.length === 0 ? (
            <div className="bg-surface border border-border rounded-xl py-12 text-center text-muted text-sm">
              No predictions recorded yet. Predictions are generated by the ML model during
              inference.
            </div>
          ) : (
                  ...
                  <tbody>
                    {predictions.map((p) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-2 text-center">
                          <span
                            className={`text-xs ${p.confidence === "high" ? "text-success" : p.confidence === "medium" ? "text-warning" : "text-muted"}`}
                          >
                            {p.confidence}
                          </span>
                        </td>
