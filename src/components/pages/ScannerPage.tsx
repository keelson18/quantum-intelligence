...
export default function ScannerPage() {
  ...
  const [sortBy, setSortBy] = useState<
    "aiScore" | "score" | "change" | "confidence" | "volume" | "volatility"
  >("aiScore");
      ...
      await Promise.all(
        batch.map(async (sym) => {
          try {
            ...
            const change =
              ((candles[lastIdx].close - candles[lastIdx - 24].close) /
                candles[lastIdx - 24].close) *
              100;
            ...
            const recentPattern = patterns.find(
              (p) => p.index >= lastIdx - 3 && p.side !== "neutral",
            );
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted mt-0.5">
            Multi-factor scan with AI Score ranking across{" "}
            {CRYPTO_INSTRUMENTS.filter((i) => i.live).length} instruments
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4 text-xs">
        ...
        <div className="flex items-center gap-1">
          ...
          <input
            ...
            placeholder="min"
            className="w-12 px-1.5 py-1 rounded bg-surface border border-border text-text text-xs tabular-nums"
          />
        ...
        <div className="flex items-center gap-1">
          ...
          <input
            ...
            placeholder="0"
            className="w-16 px-1.5 py-1 rounded bg-surface border border-border text-text text-xs tabular-nums"
          />
        </div>
        <div className="flex items-center gap-1">
          ...
          <input
            value={aiConfMin}
            onChange={(e) => setAiConfMin(e.target.value)}
            placeholder="%"
            className="w-12 px-1.5 py-1 rounded bg-surface border border-border text-text text-xs tabular-nums"
          />
        </div>
                  ...
                  >
                    <td className="px-4 py-2.5 font-medium">
                      <span className="flex items-center gap-1.5">
                        {idx < 3 && (
                          <span
                            className={`w-5 h-5 rounded text-[10px] font-bold flex items-center justify-center ${idx === 0 ? "bg-primary text-black" : idx === 1 ? "bg-primary/60 text-black" : "bg-primary/30 text-primary"}`}
                          >
                            {idx + 1}
                          </span>
                        )}
                    ...
                    <td
                      className={`px-4 py-2.5 text-right tabular-nums ${r.change24h >= 0 ? "text-success" : "text-danger"}`}
                    >
                      {r.change24h >= 0 ? "+" : ""}
                    ...
                    <td className="px-4 py-2.5 text-right tabular-nums">
                      <span
                        className={`font-bold ${r.aiScore >= 80 ? "text-success" : r.aiScore >= 60 ? "text-primary" : r.aiScore >= 40 ? "text-warning" : "text-muted"}`}
                      >
                        {r.aiScore}
                      </span>
                    </td>
                    ...
                    <td
                      className={`px-4 py-2.5 text-right tabular-nums ${r.rsi > 70 ? "text-danger" : r.rsi < 30 ? "text-success" : ""}`}
                    >
                      {r.rsi.toFixed(0)}
                    </td>
                    <td
                      className={`px-4 py-2.5 text-right tabular-nums ${r.adx > 25 ? "text-primary" : "text-muted"}`}
                    >
                      {r.adx.toFixed(0)}
                    </td>
                    <td
                      className={`px-4 py-2.5 text-right tabular-nums ${r.macdHist > 0 ? "text-success" : "text-danger"}`}
                    >
                      {r.macdHist.toFixed(4)}
                    </td>
                    <td className="px-4 py-2.5 text-center">
                      {r.emaCross !== "none" ? (
                        <span
                          className={`text-[10px] font-medium ${r.emaCross === "bullish" ? "text-success" : "text-danger"}`}
                        >
                          {r.emaCross === "bullish" ? "↑" : "↓"}
                        </span>
                      ) : (
