...
export default function ResearchPage() {
  ...
  const [corrMatrix, setCorrMatrix] = useState<{ symbols: string[]; data: number[][] } | null>(
    null,
  );
  ...
  const runAnalysis = useCallback(async () => {
    ...
    for (const sym of symbols) {
      ...
      try {
        ...
        const change24h =
          ((candles1h[lastIdx].close - candles1h[lastIdx - 24].close) /
            candles1h[lastIdx - 24].close) *
          100;
        ...
        const trend: "Bullish" | "Bearish" | "Neutral" =
          structure.regime === "trend_up" || (macdHist > 0 && rsi > 50)
            ? "Bullish"
            : structure.regime === "trend_down" || (macdHist < 0 && rsi < 50)
              ? "Bearish"
              : "Neutral";
    ...
    <div className="px-4 lg:px-6 py-4 space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted mt-0.5">
            Market intelligence hub — asset reports, AI outlook, correlation matrix
          </p>
        </div>
        <button
          onClick={runAnalysis}
      ...
      <div>
        <h3 className="text-xs font-semibold mb-2 flex items-center gap-1.5">
          <FileText className="w-3.5 h-3.5 text-primary" /> AI Asset Reports
        </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {reports.map((r) => (
              ...
              >
                ...
                {r.pattern && (
                  <div className="mt-2 text-xs text-primary bg-primary/5 rounded-lg px-2 py-1">
                    Pattern: {r.pattern}
                  </div>
                )}
                <div className="mt-1.5 text-xs text-muted">
                  Strategy: {r.strategy} · {REGIME_LABELS[r.regime]}
                </div>
              </div>
            ))}
          </div>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-border text-muted">
                  ...
                  {SCAN_TFS.map((tf) => (
                    <th key={tf} className="text-center px-4 py-2.5 font-medium" colSpan={3}>
                      {tf}
                    </th>
                  ))}
                      ...
                      return (
                        <Fragment key={tf}>
                          <td className="px-2 py-2.5 text-center">
                            {regime ? (
                              <span className={REGIME_COLORS[regime]}>{REGIME_LABELS[regime]}</span>
                            ) : (
                              "—"
                            )}
                          </td>
                      ...
                      <div className="flex items-center justify-end gap-1">
                        <div className="w-16 h-1.5 rounded-full bg-bg overflow-hidden">
                          <div
                            ...
                            style={{ width: `${r.trendStrength * 100}%` }}
                          />
                        </div>
                        <span className="tabular-nums text-muted">
                          {(r.trendStrength * 100).toFixed(0)}%
                        </span>
                      </div>
                    </td>
                  </tr>
              ...
              <thead>
                <tr className="border-b border-border text-muted">
                  ...
                  {corrMatrix.symbols.map((s) => (
                    <th key={s} className="px-3 py-2 font-medium text-center">
                      {s.replace("USDT", "")}
                    </th>
                  ))}
                </tr>
...
function ReportPill({
  ...
  return (
    <div className="bg-bg/50 rounded-lg p-2 border border-border/50">
      <div className="text-[10px] text-muted mb-0.5 flex items-center gap-0.5">
        {icon}
        {label}
      </div>
