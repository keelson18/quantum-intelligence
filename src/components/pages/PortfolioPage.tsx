...
export default function PortfolioPage() {
  ...
  if (loading) {
    return (
      <div className="px-4 lg:px-6 py-12 text-center text-muted text-sm">Loading portfolio…</div>
    );
  }
    ...
    <div className="px-4 lg:px-6 py-4 space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted mt-0.5">
            Paper trading positions, analytics & performance
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
        ...
        <MetricCard
          ...
          sub={`${(metrics.totalReturnPct * 100).toFixed(1)}%`}
          positive={metrics.totalReturn >= 0}
        />
        <MetricCard
          label="Unrealized P&L"
          value={`${unrealized.totalPnl >= 0 ? "+" : ""}$${unrealized.totalPnl.toFixed(0)}`}
          positive={unrealized.totalPnl >= 0}
        />
        <MetricCard
          label="Win Rate"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
        ...
        <MetricCard
          label="Avg Hold"
          value={`${metrics.avgHoldHours.toFixed(1)}h`}
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            <div>
              ...
              <div className="text-lg font-semibold tabular-nums">
                ${exposure.totalExposure.toFixed(0)}
              </div>
            </div>
            <div>
              ...
              <div className="text-lg font-semibold tabular-nums text-success">
                ${exposure.bySide.long.toFixed(0)}
              </div>
            </div>
            <div>
              <div className="text-xs text-muted flex items-center gap-1">
              ...
              <div className="text-lg font-semibold tabular-nums text-danger">
                ${exposure.bySide.short.toFixed(0)}
              </div>
            </div>
          </div>
              ...
              {exposure.byAsset.map((a) => (
                <div key={a.symbol} className="flex items-center gap-3 text-xs">
                  ...
                  <div className="flex-1 h-2 rounded-full bg-bg overflow-hidden">
                    <div
                      className={`h-full ${a.side === "long" ? "bg-success" : "bg-danger"}`}
                      style={{ width: `${Math.min(100, a.pct * 100)}%` }}
                    />
                  </div>
      ...
      <div className="bg-surface border border-border rounded-xl overflow-hidden">
        ...
        {livePositions.length === 0 ? (
          <div className="py-10 text-center text-muted text-sm">
            No open positions. Execute trades from the Terminal.
          </div>
        ) : (
                ...
                {livePositions.map((p) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5 text-center">
                      <span
                        className={`px-2 py-0.5 rounded text-xs font-medium ${p.side === "long" ? "bg-success/15 text-success" : "bg-danger/15 text-danger"}`}
                      >
                        {p.side}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-2.5 text-right tabular-nums">
                      {livePrices[p.symbol] ? (
                        <span className="text-primary">${p.currentPrice.toFixed(2)}</span>
                      ) : (
                        `$${p.currentPrice.toFixed(2)}`
                      )}
                    </td>
                    <td className="px-4 py-2.5 text-right tabular-nums">{p.quantity}</td>
                    <td className="px-4 py-2.5 text-right tabular-nums text-danger/70">
                      {p.stop_loss ? `$${p.stop_loss.toFixed(2)}` : "—"}
                    </td>
                    <td className="px-4 py-2.5 text-right tabular-nums text-success/70">
                      {p.take_profit ? `$${p.take_profit.toFixed(2)}` : "—"}
                    </td>
                    <td
                      className={`px-4 py-2.5 text-right tabular-nums font-medium ${p.pnl >= 0 ? "text-success" : "text-danger"}`}
                    >
                      {p.pnl >= 0 ? "+" : ""}${p.pnl.toFixed(2)}
                    </td>
                    <td
                      className={`px-4 py-2.5 text-right tabular-nums ${p.pnlPct >= 0 ? "text-success" : "text-danger"}`}
                    >
                      {p.pnlPct >= 0 ? "+" : ""}
                ...
                {trades.map((t) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2 text-center">
                      <span
                        className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${t.side === "long" ? "bg-success/15 text-success" : "bg-danger/15 text-danger"}`}
                      >
                        {t.side}
                      </span>
                    </td>
                    ...
                    <td
                      className={`px-4 py-2 text-right tabular-nums font-medium ${t.pnl >= 0 ? "text-success" : "text-danger"}`}
                    >
                      {t.pnl >= 0 ? "+" : ""}${t.pnl.toFixed(2)}
                    </td>
                    <td
                      className={`px-4 py-2 text-right tabular-nums ${t.pnl_pct >= 0 ? "text-success" : "text-danger"}`}
                    >
                      {t.pnl_pct >= 0 ? "+" : ""}
