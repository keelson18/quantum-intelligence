    ...
    <div className="px-4 lg:px-6 py-4 max-w-2xl">
      ...
      <Section icon={Sliders} title="Trading Preferences">
        <div className="flex items-center justify-between py-2">
          ...
          <select
            value={store.defaultTimeframe}
            onChange={(e) =>
              store.setSetting("defaultTimeframe", e.target.value as typeof store.defaultTimeframe)
            }
        ...
        <div className="flex items-center justify-between py-2 border-t border-border/50">
          ...
          <select
            ...
            onChange={(e) => store.setSetting("riskTolerance", e.target.value as RiskTolerance)}
            className="px-3 py-2 rounded-lg bg-bg border border-border text-text text-sm focus:outline-none focus:border-primary"
          >
        ...
        <div className="flex items-center justify-between py-2 border-t border-border/50">
          ...
          <input
            type="number"
            value={store.portfolioEquity}
            onChange={(e) => store.setSetting("portfolioEquity", parseFloat(e.target.value) || 0)}
        ...
        <div className="flex items-center justify-between py-2 border-t border-border/50">
          ...
          <input
            type="number"
            step="0.5"
            value={store.maxRiskPerTrade * 100}
            onChange={(e) =>
              store.setSetting("maxRiskPerTrade", (parseFloat(e.target.value) || 0) / 100)
            }
      ...
      <Section icon={Bell} title="Notifications">
        <Toggle
          ...
          value={store.notifPriceAlerts}
          onChange={(v) => store.setSetting("notifPriceAlerts", v)}
        />
        <Toggle
          label="AI Signal Alerts"
          description="Get notified when the AI generates a new trade signal"
          value={store.notifAISignals}
          onChange={(v) => store.setSetting("notifAISignals", v)}
        />
        <Toggle
          label="Risk Warnings"
          description="Get alerted when portfolio risk limits are breached"
          value={store.notifRiskWarnings}
          onChange={(v) => store.setSetting("notifRiskWarnings", v)}
        />
        <Toggle
          label="Strategy Triggers"
          description="Get notified when a strategy fires a signal"
          value={store.notifStrategyTriggers}
          onChange={(v) => store.setSetting("notifStrategyTriggers", v)}
        />
      </Section>
