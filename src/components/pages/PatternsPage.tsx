      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted mt-0.5">
            Scanning all instruments for chart and candlestick patterns
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {loading && rows.length === 0 ? (
          ...
          <div className="col-span-full py-12 text-center text-muted text-sm">
            No patterns detected with the current filter.
          </div>
        ) : (
          filtered.map((r) => (
            <div
              ...
              <div className="space-y-1.5">
                {r.patterns.map((p, i) => (
                  <div key={i} className="flex items-center gap-2 text-xs">
                    <span
                      className={`w-1.5 h-1.5 rounded-full ${p.side === "buy" ? "bg-success" : p.side === "sell" ? "bg-danger" : "bg-muted"}`}
                    />
                    <span className="font-medium">{p.name}</span>
                    <span
                      className={`px-1.5 py-0.5 rounded text-[10px] ${p.kind === "chart" ? "bg-primary/10 text-primary" : "bg-warning/10 text-warning"}`}
                    >
                      {p.kind}
                    </span>
