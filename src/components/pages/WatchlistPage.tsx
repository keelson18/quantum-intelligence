...
export default function WatchlistPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    if (lists.length === 0) {
      ...
      if (created.length > 0) {
        ...
        if (cryptoList) {
          const cryptoItems = CRYPTO_INSTRUMENTS.filter((i) => i.live)
            .slice(0, 6)
            .map((inst, idx) => ({
              watchlist_id: cryptoList.id,
              symbol: inst.symbol,
              label: inst.label,
              market: "crypto",
              display_order: idx,
  ...
  const addItem = async (symbol: string, label: string, market: string) => {
    ...
    await addWatchlistItems([
      { watchlist_id: activeListId, symbol, label, market, display_order: order },
    ]);
    setShowAdd(false);
    setSearch("");
    setItems((await listWatchlistItems(activeListId)) as WatchlistItem[]);
  ...
  if (loading)
    return (
      <div className="px-4 lg:px-6 py-12 text-center text-muted text-sm">Loading watchlists…</div>
    );
    ...
    <div className="px-4 lg:px-6 py-4 space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted mt-0.5">
            Track assets across crypto, forex, commodities & more
          </p>
        </div>
        <button
      ...
      {showNewList && (
        <div className="flex items-center gap-2 bg-surface border border-border rounded-lg p-2">
          ...
          <button
            onClick={createList}
            className="px-3 py-1.5 rounded bg-primary text-black text-xs font-medium"
          >
            Create
          </button>
          <button onClick={() => setShowNewList(false)} className="text-muted hover:text-text">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
      ...
      {showAdd && (
        <div className="bg-surface border border-border rounded-xl p-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setShowAdd(false)} className="text-muted hover:text-text">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="max-h-48 overflow-y-auto space-y-1">
            {filteredInstruments.slice(0, 20).map((inst) => (
              <button
                key={inst.symbol}
                onClick={() => addItem(inst.symbol, inst.label, inst.market)}
                ...
                .map((item, idx) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-3 py-2.5">
                        <div className="flex flex-col items-center gap-0.5">
                          <button
                            ...
                            disabled={idx === 0}
                            className="text-muted hover:text-text disabled:opacity-30 text-[10px]"
                          >
                            ▲
                          </button>
                      ...
                      <td className="px-4 py-2.5 text-center text-muted capitalize">
                        {item.market}
                      </td>
