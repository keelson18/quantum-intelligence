...
export default function RiskPage() {
  ...
  const longExposure = rows
    .filter((r) => r.side === "buy")
    .reduce((s, r) => s + r.positionValue, 0);
    ...
    <div className="px-4 lg:px-6 py-4">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted mt-0.5">
            Kelly Criterion sizing, exposure limits, and volatility-adjusted stops
          </p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setPortfolio({ ...portfolio, equity: parseFloat(e.target.value) || 0 })
              }
          ...
          <div>
            ...
            <input
              type="number"
              step="0.5"
              value={portfolio.maxDailyLossPct * 100}
              onChange={(e) =>
                setPortfolio({
                  ...portfolio,
                  maxDailyLossPct: (parseFloat(e.target.value) || 0) / 100,
                })
              }
              className="w-full px-3 py-2 rounded-lg bg-bg border border-border text-text text-sm tabular-nums focus:outline-none focus:border-primary"
            />
          ...
          <div>
            ...
            <input
              type="number"
              step="1"
              value={portfolio.maxDrawdownPct * 100}
              onChange={(e) =>
                setPortfolio({
                  ...portfolio,
                  maxDrawdownPct: (parseFloat(e.target.value) || 0) / 100,
                })
              }
              className="w-full px-3 py-2 rounded-lg bg-bg border border-border text-text text-sm tabular-nums focus:outline-none focus:border-primary"
            />
          ...
          <div>
            ...
            <input
              type="number"
              step="5"
              value={portfolio.maxExposurePct * 100}
              onChange={(e) =>
                setPortfolio({
                  ...portfolio,
                  maxExposurePct: (parseFloat(e.target.value) || 0) / 100,
                })
              }
              className="w-full px-3 py-2 rounded-lg bg-bg border border-border text-text text-sm tabular-nums focus:outline-none focus:border-primary"
            />
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-3 mb-4">
        <RiskCard
          ...
          sub={`${((totalExposure / portfolio.equity) * 100).toFixed(1)}% of equity`}
        />
        <RiskCard
          label="Total Risk"
          value={`$${totalRisk.toFixed(0)}`}
          sub={`${((totalRisk / portfolio.equity) * 100).toFixed(1)}% at risk`}
          warn={totalRisk > maxDailyLoss}
        />
        <RiskCard
          label="Avg Kelly"
          value={`${(avgKelly * 100).toFixed(1)}%`}
          sub="Quarter-Kelly cap"
        />
        <RiskCard
          label="Daily Loss Limit"
          value={`$${maxDailyLoss.toFixed(0)}`}
          sub={`${(portfolio.maxDailyLossPct * 100).toFixed(1)}% max`}
        />
        <RiskCard
          label="Current DD"
          value={`${(currentDrawdown * 100).toFixed(1)}%`}
          sub={`Limit ${(portfolio.maxDrawdownPct * 100).toFixed(0)}%`}
          warn={currentDrawdown > portfolio.maxDrawdownPct * 0.8}
        />
        <RiskCard
          label="Long/Short"
          value={`${longExposure.toFixed(0)}/${shortExposure.toFixed(0)}`}
          sub="Exposure split"
        />
      </div>
              ...
              <tbody>
                {rows.map((r) => (
                  ...
                  >
                    ...
                    <td
                      className={`px-4 py-2.5 text-right tabular-nums ${r.atrPct > 5 ? "text-warning" : ""}`}
                    >
                      {r.atrPct.toFixed(2)}%
                    </td>
...
function RiskCard({
  ...
  return (
    <div
      className={`bg-surface border rounded-xl p-4 ${warn ? "border-danger/30" : "border-border"}`}
    >
      <div className="text-xs text-muted mb-1 flex items-center gap-1">
        {warn && <AlertTriangle className="w-3 h-3 text-danger" />} {label}
      </div>
      <div className={`text-lg font-semibold tabular-nums ${warn ? "text-danger" : ""}`}>
        {value}
      </div>
