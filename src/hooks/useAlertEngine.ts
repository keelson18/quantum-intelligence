...
export function useAlertEngine() {
      ...
      if (recent.length > 1) {
        ...
        if (Math.abs(changePct) > PRICE_ALERT_THRESHOLD) {
          ...
          createAlert(
            "price",
            symbol,
            `${symbol} ${direction}d ${Math.abs(changePct).toFixed(2)}% in 5 min`,
            "warning",
            { changePct, currentPrice: price },
          );
      ...
      for (const rule of rulesRef.current) {
        ...
        if (rule.type === "price_above" && rule.threshold && price >= rule.threshold) {
          createAlert(
            "price",
            symbol,
            rule.message || `${symbol} above ${rule.threshold}`,
            rule.severity,
            { price, threshold: rule.threshold },
          );
        }
        if (rule.type === "price_below" && rule.threshold && price <= rule.threshold) {
          createAlert(
            "price",
            symbol,
            rule.message || `${symbol} below ${rule.threshold}`,
            rule.severity,
            { price, threshold: rule.threshold },
          );
        }
  ...
  const checkTechnicalAlert = useCallback(
    (symbol: string, candles: Candle[]) => {
      ...
      for (const rule of rulesRef.current) {
        ...
        if (rule.type === "rsi_above" && rule.threshold && rsi >= rule.threshold) {
          createAlert(
            "strategy",
            symbol,
            rule.message || `${symbol} RSI ${rsi.toFixed(0)} above ${rule.threshold}`,
            rule.severity,
            { rsi },
          );
        }
        if (rule.type === "rsi_below" && rule.threshold && rsi <= rule.threshold) {
          createAlert(
            "strategy",
            symbol,
            rule.message || `${symbol} RSI ${rsi.toFixed(0)} below ${rule.threshold}`,
            rule.severity,
            { rsi },
          );
        }
      ...
      if (ema20Prev <= ema50Prev && ema20 > ema50) {
        createAlert("strategy", symbol, `${symbol} EMA20 crossed above EMA50 (bullish)`, "info", {
          ema20,
          ema50,
        });
      } else if (ema20Prev >= ema50Prev && ema20 < ema50) {
        createAlert("strategy", symbol, `${symbol} EMA20 crossed below EMA50 (bearish)`, "info", {
          ema20,
          ema50,
        });
      }
  ...
  const checkPatternAlert = useCallback(
    (symbol: string, candles: Candle[]) => {
      ...
      for (const p of recentPatterns) {
        ...
        createAlert(
          "strategy",
          symbol,
          `${symbol} pattern detected: ${p.name} (${p.side})`,
          "info",
          { pattern: p.name, side: p.side },
        );
      }
    },
    [createAlert],
  ...
  const checkStructureAlert = useCallback(
    (symbol: string, candles: Candle[]) => {
      if (candles.length < 60) return;
      ...
      for (const e of recentEvents) {
        if (e.type === "BOS") {
          createAlert(
            "strategy",
            symbol,
            `${symbol} Break of Structure (${e.direction})`,
            "warning",
            { type: "BOS", direction: e.direction },
          );
        } else if (e.type === "CHoCH") {
          createAlert(
            "strategy",
            symbol,
            `${symbol} Change of Character (${e.direction})`,
            "warning",
            { type: "CHoCH", direction: e.direction },
          );
        }
      }
    },
  ...
  const checkAIAlert = useCallback(
    (symbol: string, candles: Candle[]) => {
      if (candles.length < 60) return;
      ...
      for (const rule of rulesRef.current) {
        ...
        if (rule.type === "ai_buy" && side === "buy") {
          createAlert(
            ...
            symbol,
            rule.message ||
              `${symbol} AI BUY signal (${(confidence * 100).toFixed(0)}% confidence)`,
            "info",
            { side, confidence },
          );
        }
        if (rule.type === "ai_sell" && side === "sell") {
          createAlert(
            ...
            symbol,
            rule.message ||
              `${symbol} AI SELL signal (${(confidence * 100).toFixed(0)}% confidence)`,
            "info",
            { side, confidence },
          );
        }
        if (rule.type === "ai_confidence" && rule.threshold && confidence * 100 >= rule.threshold) {
          createAlert(
            ...
            symbol,
            rule.message ||
              `${symbol} AI confidence ${(confidence * 100).toFixed(0)}% >= ${rule.threshold}%`,
            rule.severity,
            { confidence },
          );
        }
  ...
  const checkRiskAlert = useCallback(
    (symbol: string, candles: Candle[]) => {
      if (candles.length < 60) return;
      const decision = makeDecision(candles, null, symbol, "1h");
      ...
      if (risk.portfolioExposure > DEFAULT_PORTFOLIO.maxExposurePct) {
        createAlert(
          "risk",
          symbol,
          `Excessive exposure: ${(risk.portfolioExposure * 100).toFixed(1)}% exceeds ${(DEFAULT_PORTFOLIO.maxExposurePct * 100).toFixed(0)}% limit`,
          "critical",
          { exposure: risk.portfolioExposure },
        );
      }
      const drawdown =
        (DEFAULT_PORTFOLIO.peakEquity - DEFAULT_PORTFOLIO.equity) / DEFAULT_PORTFOLIO.peakEquity;
      if (drawdown > DEFAULT_PORTFOLIO.maxDrawdownPct * 0.8) {
        createAlert(
          "risk",
          symbol,
          `Drawdown warning: ${(drawdown * 100).toFixed(1)}% approaching ${(DEFAULT_PORTFOLIO.maxDrawdownPct * 100).toFixed(0)}% limit`,
          "critical",
          { drawdown },
        );
      }
