...
export function candleSamples(candles: Candle[], horizon = 5): TrainingSample[] {
  ...
  const nearOrderBlock = (i: number) => {
    ...
    return smc.orderBlocks.some((ob) => Math.abs(price - (ob.high + ob.low) / 2) / price < 0.01)
      ? 1
      : 0;
  };
...
export function normalize(samples: TrainingSample[]): {
  normalized: TrainingSample[];
  mean: number[];
  std: number[];
} {
...
export async function prepareDataset(
  ...
  if (combined.length < 20) {
    throw new Error(
      `Insufficient training data: only ${combined.length} samples for ${symbol}. Need at least 20.`,
    );
  }
...
function movingAverage(vals: number[], period: number): number[] {
  return vals.map((_, i) =>
    i < period - 1 ? 0 : vals.slice(i - period + 1, i + 1).reduce((a, b) => a + b, 0) / period,
  );
}
