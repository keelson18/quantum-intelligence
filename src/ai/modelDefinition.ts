...
export function buildDenseModel(hp: Hyperparams, numFeatures: number): tf.LayersModel {
  ...
  for (let i = 0; i < hp.hiddenUnits.length; i++) {
    x = tf.layers
      .dense({ units: hp.hiddenUnits[i], activation: "relu", name: `dense_${i}` })
      .apply(x) as tf.SymbolicTensor;
    if (hp.dropout > 0) {
      x = tf.layers
        .dropout({ rate: hp.dropout, name: `dropout_${i}` })
        .apply(x) as tf.SymbolicTensor;
    }
  }
  const output =
    hp.taskType === "classification"
      ? (tf.layers
          .dense({ units: hp.numClasses, activation: "softmax", name: "output" })
          .apply(x) as tf.SymbolicTensor)
      : (tf.layers
...
export function buildTransformerModel(hp: Hyperparams, numFeatures: number): tf.LayersModel {
  ...
  const x = tf.layers
    .dense({ units: hp.hiddenUnits[0] ?? 64, activation: "relu", name: "proj" })
  ...
  const attnWeights = tf.layers
    .dense({ units: 1, activation: "softmax", name: "attn_weights" })
    .apply(x) as tf.SymbolicTensor;
  ...
  const pooled = tf.layers
    .globalAveragePooling1d({ name: "pool" })
    .apply(weighted) as tf.SymbolicTensor;

  // Feed-forward head.
  let head = pooled;
  for (let i = 1; i < hp.hiddenUnits.length; i++) {
    head = tf.layers
      .dense({ units: hp.hiddenUnits[i], activation: "relu", name: `ff_${i}` })
...
export function buildModel(
  ...
  switch (architecture) {
    case "dense":
      return buildDenseModel(hp, numFeatures);
    case "lstm":
      return buildLSTMModel(hp, numFeatures);
    case "transformer":
      return buildTransformerModel(hp, numFeatures);
    default:
