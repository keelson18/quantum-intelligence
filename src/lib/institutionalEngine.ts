...
export function analyzeMultiTimeframe(
  ...
  const tfResults = tfs.map((tf) => {
    ...
    const trend: "bullish" | "bearish" | "neutral" =
      structure.regime === "trend_up" || (ema20 > ema50 && close > ema20)
        ? "bullish"
        : structure.regime === "trend_down" || (ema20 < ema50 && close < ema20)
          ? "bearish"
          : "neutral";
    ...
    const structLabel: "bos_bullish" | "bos_bearish" | "choch_bullish" | "choch_bearish" | "none" =
      !lastEvent
        ? "none"
        : lastEvent.type === "BOS" && lastEvent.direction === "bullish"
          ? "bos_bullish"
          : lastEvent.type === "BOS" && lastEvent.direction === "bearish"
            ? "bos_bearish"
            : lastEvent.type === "CHoCH" && lastEvent.direction === "bullish"
              ? "choch_bullish"
              : lastEvent.type === "CHoCH" && lastEvent.direction === "bearish"
                ? "choch_bearish"
                : "none";
  ...
  const alignedDirection: Side =
    bullCount > bearCount && bullCount >= Math.ceil(total * 0.6)
      ? "buy"
      : bearCount > bullCount && bearCount >= Math.ceil(total * 0.6)
        ? "sell"
        : "neutral";

  const summary =
    tfResults.map((t) => t.label).join(" → ") +
...
export function classifyGranularRegime(
  ...
  const returns = closes
    .slice(-21)
    .map((c, i) => (i > 0 ? Math.log(c / closes[closes.length - 21 + i - 1]) : 0))
    .slice(1);
    ...
    case "rsi_overbought":
      if (regime === "strong_uptrend" || regime === "weak_uptrend")
        return {
          ...
          reason: "RSI overbought in uptrend — momentum continuation, not reversal",
        };
      if (regime === "range_bound")
        return {
          side: "sell",
          confidenceMultiplier: 1.0,
          reason: "RSI overbought in range — mean reversion likely",
        };
      return {
        side: "sell",
        confidenceMultiplier: 0.6,
        reason: "RSI overbought — potential reversal",
      };
    case "rsi_oversold":
      if (regime === "strong_downtrend" || regime === "weak_downtrend")
        return {
          side: "sell",
          confidenceMultiplier: 0.8,
          reason: "RSI oversold in downtrend — momentum continuation, not bounce",
        };
      if (regime === "range_bound")
        return {
          side: "buy",
          confidenceMultiplier: 1.0,
          reason: "RSI oversold in range — mean reversion likely",
        };
    ...
    case "macd_bullish":
      if (regime === "strong_uptrend")
        return {
          ...
          reason: "MACD bullish in strong uptrend — high probability continuation",
        };
      if (regime === "range_bound")
        return {
          side: "buy",
          confidenceMultiplier: 0.5,
          reason: "MACD bullish in range — weak signal, may fail",
        };
    ...
    case "macd_bearish":
      if (regime === "strong_downtrend")
        return {
          side: "sell",
          confidenceMultiplier: 1.2,
          reason: "MACD bearish in strong downtrend — high probability continuation",
        };
      if (regime === "range_bound")
        return {
          side: "sell",
          confidenceMultiplier: 0.5,
          reason: "MACD bearish in range — weak signal",
        };
...
export function analyzeLiquidity(candles: Candle[]): LiquidityAnalysis {
  ...
  const buySideLiquidity = highs
    ...
    .map((h) => {
      ...
      const strength =
        touches >= 3 ? ("high" as const) : touches >= 2 ? ("medium" as const) : ("low" as const);
  ...
  const liquidityScore = Math.min(
    1,
    poolCount * 0.12 + equalCount * 0.15 + (sweepDetected ? 0.3 : 0),
  );
...
export function scoreConfluence(inputs: ConfluenceInputs): ConfluenceBreakdown {
  ...
  const timeframeAlignment = Math.round(
    inputs.timeframeScore * CONFLUENCE_WEIGHTS.timeframeAlignment,
  );
...
export function gradeTrade(
  ...
  const blended =
    confluence.total * 0.5 + confidence * 100 * 0.3 + rrScore * 0.2 * (regimeAlignment ? 1 : 0.7);

  const grade =
    blended >= 90 ? "A+" : blended >= 78 ? "A" : blended >= 62 ? "B" : blended >= 45 ? "C" : "D";
  const quality =
    blended >= 90
      ? "Excellent"
      : blended >= 78
...
export function analyzeMarketMemory(candles: Candle[], lookback = 90): MarketMemory {
  ...
  for (let i = 20; i < data.length - 3; i++) {
    ...
    if (data[i].close > swingHigh && data[i + 3].close < swingHigh) {
      const existing = failedBreakouts.find(
        (f) => f.direction === "up" && Math.abs(f.level - swingHigh) / swingHigh < 0.01,
      );
    ...
    if (data[i].close < swingLow && data[i + 3].close > swingLow) {
      const existing = failedBreakouts.find(
        (f) => f.direction === "down" && Math.abs(f.level - swingLow) / swingLow < 0.01,
      );
      if (existing) existing.count++;
  ...
  const historicalResistance = highs
    .slice(-10)
    .map((h) => h.value)
    .filter((v) => v > close);
  const historicalSupport = lows
    .slice(-10)
    .map((l) => l.value)
    .filter((v) => v < close);
  ...
  const reactionScore = Math.min(
    1,
    nearRejections.reduce((s, r) => s + r.count * 0.2, 0),
  );
  ...
  return {
    ...
    failedBreakouts: failedBreakouts.slice(0, 10),
    historicalSupport,
    historicalResistance,
    reactionScore,
    summary,
...
export function assessPortfolioIntelligence(
  ...
  const concentrationRisk: "low" | "medium" | "high" =
    totalExposure > 0.35 || positionCount >= 5
      ? "high"
      : totalExposure > 0.2 || positionCount >= 3
        ? "medium"
        : "low";
  ...
  if (correlationRisk === "high") {
    multiplier *= 0.5;
    parts.push(
      `High correlation (${(maxCorr * 100).toFixed(0)}%) with existing positions — reduce size by 50%`,
    );
  ...
  if (concentrationRisk === "high") {
    multiplier *= 0.6;
    parts.push(
      `High concentration (${(totalExposure * 100).toFixed(0)}% exposure, ${positionCount} positions) — reduce size`,
    );
...
export function computeCorrelationMatrix(seriesMap: Record<string, number[]>): {
  symbols: string[];
  matrix: number[][];
} {
...
export function analyzeMarketContext(
  ...
  const nearestResistance =
    resistanceLevels.length > 0
      ? {
          level: resistanceLevels.reduce((min, v) => (v < min ? v : min), Infinity),
          distancePct: 0,
        }
      : null;
  if (nearestResistance)
    nearestResistance.distancePct = ((nearestResistance.level - close) / close) * 100;
  ...
  if (side === "buy" && nearestResistance && nearestResistance.distancePct < 0.5) {
    contextPenalty += 0.15;
    parts.push(
      `Resistance ${nearestResistance.distancePct.toFixed(1)}% away — buying into resistance`,
    );
  }
  ...
  if (liquidity.sweepDetected) {
    if (
      (side === "buy" && liquidity.sweepDirection === "sell_side") ||
      (side === "sell" && liquidity.sweepDirection === "buy_side")
    ) {
...
export function runInstitutionalPipeline(
  ...
  const volumeScore = Math.min(
    1,
    (volRatio > 1.5 ? 0.6 : volRatio > 1.1 ? 0.4 : 0.2) + (volumeSpike ? 0.3 : 0),
  );
  ...
  const portfolioIntelligence = assessPortfolioIntelligence(
    symbol,
    side,
    Math.abs(baseRecommendation.score),
    portfolio,
    correlationSeries,
    candles,
  );
  ...
  const structureScore =
    structure.events.length > 0 ? Math.min(1, 0.5 + structure.events.length * 0.15) : 0.3;
  const rsi = ind.rsi[lastIdx] ?? 50;
  const macdHist = ind.macd.hist[lastIdx] ?? 0;
  const indicatorScore = Math.min(
    1,
    Math.abs(rsi - 50) / 30 + (Math.abs(macdHist) / (ind.atr[lastIdx] || 1)) * 0.3,
  );
  const patternScore =
    patterns.length > 0
      ? Math.min(
          1,
          patterns.reduce((s, p) => s + p.confidence, 0),
        )
      : 0.1;
  const srScore =
    marketContext.nearestSupport && marketContext.nearestResistance
      ? 1 -
        Math.min(
          1,
          (marketContext.nearestSupport.distancePct + marketContext.nearestResistance.distancePct) /
            10,
        )
      : 0.3;
  const riskScore = baseRecommendation.risk
    ? Math.min(1, baseRecommendation.risk.riskReward / 3)
    : 0.3;
  ...
  const confluence = scoreConfluence({
    ...
    srScore,
    timeframeScore: multiTimeframe.alignmentScore,
    riskScore,
  });
  ...
  const mtfPenalty =
    multiTimeframe.alignedDirection !== "neutral" && multiTimeframe.alignedDirection !== side
      ? 0.1
      : 0;
  ...
  const regimeAlignment =
    side === "buy"
      ? granularRegime === "strong_uptrend" ||
        granularRegime === "weak_uptrend" ||
        granularRegime === "accumulation"
      : side === "sell"
        ? granularRegime === "strong_downtrend" ||
          granularRegime === "weak_downtrend" ||
          granularRegime === "distribution"
        : true;
  ...
  evidence.push(
    `Multi-timeframe alignment: ${(multiTimeframe.alignmentScore * 100).toFixed(0)}% (${multiTimeframe.alignedDirection})`,
  );
  evidence.push(`Confluence: ${confluence.total}/100 (Grade ${confluence.grade})`);
  evidence.push(`Liquidity: ${liquidity.summary}`);
  evidence.push(
    `Volume: ${volumeSpike ? "spike detected" : volRatio > 1.3 ? "above average" : "normal"}, score ${(volumeScore * 100).toFixed(0)}%`,
  );
  ...
  if (ml)
    evidence.push(
      `ML: ${ml.prediction} (${(ml.probability * 100).toFixed(0)}% prob, ${ml.confidence} confidence)`,
    );
