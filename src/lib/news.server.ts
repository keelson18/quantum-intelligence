...
export async function refreshNews(admin: AdminClient) {
  ...
  try {
    const tickerRes = await fetch(
      `${serverConfig().marketRestUrl}/api/v3/ticker/24hr?symbol=BTCUSDT`,
    );
    if (tickerRes.ok) {
  ...
  headlines.push(
    {
      ...
      summary:
        "On-chain data shows a significant increase in Ethereum transaction count, with gas fees dropping to their lowest level this month. The combination suggests growing network adoption without congestion bottlenecks.",
      source: "Chain News",
    ...
    },
    {
      ...
      summary:
        "Total value locked in Solana DeFi protocols has climbed to a fresh high, driven primarily by growth in liquid staking derivatives. The ecosystem continues to attract developer activity and user liquidity.",
      source: "DeFi Pulse",
    ...
    },
    {
      ...
      summary:
        "A major financial regulator has opened a review into stablecoin issuer reserve practices, requesting detailed audits. The announcement triggered a brief flight to quality across crypto markets.",
    ...
    },
    {
      ...
      summary:
        "XRP has recorded above-average exchange inflows over the past 24 hours, often a precursor to elevated volatility. Traders are watching key support and resistance levels for directional confirmation.",
      source: "Flow Desk",
    ...
    },
    {
      ...
      summary:
        "Aggregate Layer-2 transaction throughput reached a new milestone this week, with combined daily transactions surpassing Ethereum mainnet by a wide margin. The trend highlights the shift toward scalable execution environments.",
      source: "L2 Beat",
    ...
    },
    {
      ...
      summary:
        "Chainlink revealed additional Cross-Chain Interoperability Protocol integrations with established financial institutions, expanding the bridge between traditional finance and on-chain settlement.",
      source: "Oracle Times",
    ...
    },
    {
      ...
      summary:
        "Several large market makers have tightened quotes and reduced crypto inventory in anticipation of key macroeconomic data. Liquidity has thinned modestly, which could amplify price reactions to the release.",
    ...
    },
    {
      ...
      summary:
        "A new Avalanche subnet tailored for institutional DeFi has gone live, with several pilot programs from traditional asset managers. The infrastructure targets compliance-friendly on-chain financial products.",
