  ...
  return runEngine<LiquidityResult>(D.id, D.version, contextId, () => {
    if (candles.length < 30) {
      return {
        ...
        result: null,
        confidence: 0,
        warnings: ["Insufficient history for liquidity mapping"],
      };
    }
    ...
    const labelled: LiquidityResult["labelled"] = [
      {
        claim: `${analysis.buySideLiquidity.length} buy-side pools mapped from prior highs`,
        kind: "observed",
      },
      {
        claim: `${analysis.sellSideLiquidity.length} sell-side pools mapped from prior lows`,
        kind: "observed",
      },
      {
        claim: `${analysis.equalHighs.length} equal highs / ${analysis.equalLows.length} equal lows`,
        kind: "observed",
      },
      {
        claim: analysis.sweepDetected
          ? `Sweep of ${analysis.sweepDirection.replace("_", " ")} liquidity`
          : "No sweep detected",
        kind: "inferred",
      },
      {
        claim: `Sweep probability ${(analysis.sweepProbability * 100).toFixed(0)}%`,
        kind: "inferred",
      },
    ];

    const stopRunCandidates = [
      ...analysis.buySideLiquidity
        .filter((l) => l.strength !== "low")
        .map((l) => ({ level: l.level, side: "buy_side" as const })),
      ...analysis.sellSideLiquidity
        .filter((l) => l.strength !== "low")
        .map((l) => ({ level: l.level, side: "sell_side" as const })),
    ];
    ...
    return {
      ...
      evidence,
      warnings: [
        "Order-book depth is not provided by the configured data source; liquidity is inferred from price action",
      ],
    };
  });
}
