  ...
  return runEngine<MarketContextResult>(D.id, D.version, contextId, () => {
    if (candles.length < 60) {
      return {
        ...
        result: null,
        confidence: 0,
        warnings: ["Fewer than 60 bars — context not constructed"],
      };
    }
    ...
    const higherTimeframeBias =
      mtf.alignedDirection === "buy"
        ? "bullish"
        : mtf.alignedDirection === "sell"
          ? "bearish"
          : "neutral";
    ...
    const evidence: Evidence[] = [
      ...
      { key: "context_penalty", value: Number(context.contextPenalty.toFixed(3)) },
      {
        ...
        note: `alignment ${(mtf.alignmentScore * 100).toFixed(0)}%`,
      },
    ];
    if (context.nearestResistance)
      evidence.push({
        key: "nearest_resistance",
        value: context.nearestResistance.level,
        note: `${context.nearestResistance.distancePct.toFixed(2)}% away`,
      });
    if (context.nearestSupport)
      evidence.push({
        key: "nearest_support",
        value: context.nearestSupport.level,
        note: `${context.nearestSupport.distancePct.toFixed(2)}% away`,
      });

    const warnings: string[] = [];
    if (mtf.timeframes.length < 2)
      warnings.push("Single timeframe available — multi-timeframe context is incomplete");
