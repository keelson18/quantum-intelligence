...
export function portfolioEngine(
  ...
  return runEngine<PortfolioResult>(D.id, D.version, contextId, () => {
    const intelligence = assessPortfolioIntelligence(
      symbol,
      side,
      confidence,
      portfolio,
    ...
    if (exposurePct >= maxExposure)
      rejectionReason = `Portfolio exposure ${exposurePct.toFixed(1)}% already at the ${maxExposure.toFixed(1)}% ceiling`;
    else if (intelligence.correlationRisk === "high" && intelligence.concentrationRisk === "high")
      rejectionReason = "Correlation and concentration risk are both high";
    else if (intelligence.suggestedPositionMultiplier <= 0)
      rejectionReason = "Portfolio intelligence authorises no additional size";

    const risked = portfolio.equity * 0.01 * Math.max(0, intelligence.suggestedPositionMultiplier);
    const scenarios: PortfolioScenario[] = [
      {
        name: "Setup stops out",
        equityImpactPct: portfolio.equity > 0 ? -(risked / portfolio.equity) * 100 : 0,
        detail: "Single-position stop loss at planned risk",
      },
      {
        name: "Correlated cluster stops out",
        equityImpactPct:
          portfolio.equity > 0
            ? -((risked * Math.max(1, intelligence.positionCount + 1)) / portfolio.equity) * 100
            : 0,
        detail: "All correlated positions fail together",
      },
      {
        name: "Target reached",
        equityImpactPct: portfolio.equity > 0 ? ((risked * 2) / portfolio.equity) * 100 : 0,
        detail: "Planned 2R outcome",
      },
    ];
    ...
    const evidence: Evidence[] = [
      ...
      {
        key: "suggested_multiplier",
        value: Number(intelligence.suggestedPositionMultiplier.toFixed(3)),
      },
      { key: "drawdown_pct", value: Number(drawdownPct.toFixed(2)) },
      { key: "recommendation", value: intelligence.recommendation },
      ...scenarios.map((s) => ({
        key: `scenario_${s.name.replace(/\s+/g, "_").toLowerCase()}`,
        value: Number(s.equityImpactPct.toFixed(3)),
        note: s.detail,
      })),
    ];
