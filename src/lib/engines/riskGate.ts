...
export function evaluateRiskGate(
  ...
  if (quality.score < limits.minDataQualityScore || !quality.usable) {
    violations.push({
      code: "data_quality",
      limit: limits.minDataQualityScore,
      actual: quality.score,
      detail: "Market data quality is unacceptable for execution",
    });
  }
  if (confluenceTotal !== null && confluenceTotal < limits.minConfluence) {
    violations.push({
      code: "min_confluence",
      limit: limits.minConfluence,
      actual: confluenceTotal,
      detail: "Confluence below execution floor",
    });
  }
  if (confidence < limits.minConfidence) {
    violations.push({
      code: "min_confidence",
      limit: limits.minConfidence,
      actual: confidence,
      detail: "Final confidence below execution floor",
    });
  }
  if (rec.risk) {
    if (rec.risk.riskReward < limits.minRiskReward) {
      violations.push({
        code: "min_risk_reward",
        limit: limits.minRiskReward,
        actual: rec.risk.riskReward,
        detail: "Risk/reward below minimum",
      });
    }
    ...
    if (exposurePct > limits.maxPortfolioExposurePct) {
      violations.push({
        ...
        actual: exposurePct,
        detail: "Portfolio exposure limit exceeded",
      });
    }
    ...
    if (riskPct > limits.maxRiskPerTradePct) {
      violations.push({
        ...
        limit: limits.maxRiskPerTradePct,
        actual: riskPct,
        detail: "Risk per trade limit exceeded",
      });
    }
    ...
    violations.push({
      ...
      limit: 1,
      actual: 0,
      detail: "No risk assessment available for a directional setup",
    });
  }
  if (contradictions.blocking) {
    violations.push({
      ...
      limit: 0,
      actual: contradictions.contradictions.filter((c) => c.severity === "high").length,
      detail: "High-severity contradiction present",
    });
  }
...
export function riskGateEngine(
  ...
  return runEngine("risk_gate", RISK_GATE_VERSION, contextId, () => {
    ...
    return {
      ...
      evidence: result.violations.map((v) => ({
        key: v.code,
        value: v.actual,
