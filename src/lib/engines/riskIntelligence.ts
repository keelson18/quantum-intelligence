...
export function riskIntelligenceEngine(
  ...
  return runEngine<RiskIntelligenceResult>(D.id, D.version, contextId, () => {
    const assessment =
      side === "neutral"
        ? null
        : (assessRisk(candles, side, Math.max(0.01, confidence), portfolio) ?? null);
    ...
    const dailyLossPct =
      portfolio.equity > 0 ? (portfolio.dailyLossUsed / portfolio.equity) * 100 : 0;
    const drawdownPct =
    ...
    const circuitBreakers: CircuitBreaker[] = [
      {
        code: "daily_loss",
        tripped: dailyLossPct >= maxDailyLoss,
        detail: `Daily loss ${dailyLossPct.toFixed(2)}% of ${maxDailyLoss.toFixed(1)}% budget`,
      },
      {
        code: "max_drawdown",
        tripped: drawdownPct >= maxDrawdown,
        detail: `Drawdown ${drawdownPct.toFixed(2)}% vs ${maxDrawdown.toFixed(1)}% limit`,
      },
      {
        code: "position_limit",
        tripped: exposurePct >= maxExposure,
        detail: `Exposure ${exposurePct.toFixed(1)}% vs ${maxExposure.toFixed(1)}% limit`,
      },
      {
        code: "volatility_spike",
        tripped: atrPct > 8,
        detail: `ATR ${atrPct.toFixed(2)}% of price`,
      },
    ];
    const tripped = circuitBreakers.filter((c) => c.tripped);

    const requiredAdjustments: string[] = [];
    if (assessment && assessment.riskReward < limits.minRiskReward)
      requiredAdjustments.push(`Improve risk/reward to at least ${limits.minRiskReward}:1`);
    if (tripped.some((c) => c.code === "position_limit"))
      requiredAdjustments.push("Reduce open exposure before adding risk");
    if (tripped.some((c) => c.code === "volatility_spike"))
      requiredAdjustments.push("Reduce size for elevated volatility");

    const riskScore = Math.min(
      1,
      tripped.length * 0.25 +
        (atrPct > 4 ? 0.2 : 0) +
        gate.violations.length * 0.15 +
        (drawdownPct > 5 ? 0.15 : 0),
    );
    ...
    return {
      ...
      warnings: approved
        ? []
        : [gate.reason, ...tripped.map((c) => `Circuit breaker: ${c.detail}`)],
    };
  });
}
