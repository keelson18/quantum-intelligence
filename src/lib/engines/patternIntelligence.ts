  ...
  return runEngine<PatternResult>(D.id, D.version, contextId, () => {
    if (candles.length < 30) {
      return {
        ...
        result: null,
        confidence: 0,
        warnings: ["Insufficient history for pattern detection"],
      };
    }
    ...
    const volatilityState: PatternResult["volatilityState"] =
      avgWidth === 0
        ? "neutral"
        : lastWidth < avgWidth * 0.7
          ? "compression"
          : lastWidth > avgWidth * 1.3
            ? "expansion"
            : "neutral";
    ...
    const qualified: QualifiedPattern[] = patterns.map((hit) => {
      ...
      if (family === "reversal" && trending) {
        ...
        note =
          "Reversal pattern against an established trend — context does not support acting on it alone";
