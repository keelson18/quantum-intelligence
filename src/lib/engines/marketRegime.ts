  ...
  return runEngine<RegimeResult>(D.id, D.version, contextId, () => {
    if (candles.length < 60) {
      return {
        ...
        result: null,
        confidence: 0,
        warnings: ["Insufficient history to classify regime"],
      };
    }
    ...
    const stability = samples.length
      ? samples.filter((s) => s === granular).length / samples.length
      : 0;
    ...
    const evidence: Evidence[] = [
      ...
      { key: "atr_pct", value: Number(atrPct.toFixed(3)) },
      {
        ...
        note: `${samples.length} historical windows sampled`,
      },
    ];
    ...
    return {
      ...
      result: {
        regime: baseRegime,
        granular,
        label: granularRegimeLabel(granular),
        regimeClass,
        stability,
        adx: adxVal,
        atrPct,
      ...
      evidence,
      warnings:
        stability < 0.5 ? ["Regime is unstable — treat strategy selection as provisional"] : [],
    };
  });
}
