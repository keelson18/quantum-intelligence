  ...
  return runEngine<MLResult>(D.id, D.version, contextId, () => {
    if (!ml) {
      return {
        ...
        warnings: [
          "No model prediction available — decision proceeds on deterministic evidence only",
        ],
      };
    }

    const implied: Side =
      ml.prediction === "up" ? "buy" : ml.prediction === "down" ? "sell" : "neutral";
    ...
    const agrees =
      proposedSide === "neutral" ? null : implied === proposedSide ? proposedSide : null;
