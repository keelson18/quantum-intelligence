...
export function aiReasoningEngine(
  ...
  return runEngine<ReasoningResult>(D.id, D.version, contextId, () => {
    ...
    if (input.regime) {
      statements.push({
        kind: "calculated_measurement",
        text: `Regime is ${input.regime.label} (${input.regime.regimeClass}) with ADX ${input.regime.adx.toFixed(1)} and stability ${(input.regime.stability * 100).toFixed(0)}%.`,
        source: "market_regime",
      });
      if (input.regime.regimeClass === "transition" || input.regime.regimeClass === "uncertain")
        opposing.push("Regime is unstable or unclassified");
    }
    if (input.structure) {
      ...
      statements.push({
        kind: "observed_fact",
        text: last
          ? `Last structural event: ${last.type} ${last.direction} at ${last.level.toFixed(2)}.`
          : "No recent structural break.",
        source: "market_structure",
      });
      if (
        last &&
        ((bullish && last.direction === "bullish") || (!bullish && last.direction === "bearish"))
      )
        supporting.push(`Structure confirms ${last.direction} bias`);
    }
    if (input.context) {
      statements.push({
        kind: "calculated_measurement",
        text: input.context.context.summary,
        source: "market_context",
      });
      if (input.side !== "neutral" && input.context.higherTimeframeBias !== "neutral") {
        const aligned =
          (bullish && input.context.higherTimeframeBias === "bullish") ||
          (!bullish && input.context.higherTimeframeBias === "bearish");
        (aligned ? supporting : opposing).push(
          `Higher-timeframe bias is ${input.context.higherTimeframeBias}`,
        );
      }
    }
    if (input.liquidity) {
      statements.push({
        ...
        text: input.liquidity.analysis.summary,
        source: "liquidity_intelligence",
      });
    }
    if (input.patterns) {
      statements.push({
        kind: "observed_fact",
        text: `${input.patterns.patterns.length} pattern(s) detected; ${input.patterns.qualified.filter((q) => q.contextSupported).length} are context-supported. Volatility ${input.patterns.volatilityState}.`,
    ...
    if (input.indicators) {
      statements.push({
        kind: "calculated_measurement",
        text: `Indicator balance: ${input.indicators.bullishWeight} bullish vs ${input.indicators.bearishWeight} bearish readings.`,
        source: "indicator_intelligence",
      });
      const skew = input.indicators.bullishWeight - input.indicators.bearishWeight;
      if (input.side !== "neutral")
        ((bullish && skew > 0) || (!bullish && skew < 0) ? supporting : opposing).push(
          "Indicator balance",
        );
    }
    if (input.strategy) {
      statements.push({
        kind: "calculated_measurement",
        text: `Selected strategy ${input.strategy.selected.label}: ${input.strategy.selected.reason}. ${input.strategy.rejectedCount} candidate(s) rejected.`,
    ...
    if (input.similarity && input.similarity.sampleSize > 0) {
      statements.push({
        ...
        text: `${input.similarity.sampleSize} analogous contexts: ${(input.similarity.upRate * 100).toFixed(0)}% resolved up, median ${input.similarity.medianForwardReturnPct.toFixed(2)}% (sample ${input.similarity.sampleQuality}).`,
        source: "historical_similarity",
      });
      if (input.side !== "neutral") {
        const favours = bullish
          ? input.similarity.upRate > input.similarity.downRate
          : input.similarity.downRate > input.similarity.upRate;
    ...
    if (input.knowledge) {
      for (const item of input.knowledge.items.slice(0, 3)) {
        statements.push({
          kind: item.validated
            ? ("validated_observation" as unknown as FactKind)
            : "inferred_hypothesis",
          text: item.statement,
          source: `knowledge:${item.source}`,
        });
      }
    }
    if (input.ml) {
      statements.push({
        ...
        text: `Model ${input.ml.modelVersion} predicts ${input.ml.prediction.prediction} at p=${input.ml.prediction.probability.toFixed(2)} (${input.ml.calibration} calibration).`,
        source: "ml_intelligence",
      });
      if (input.side !== "neutral")
        (input.ml.agreesWith ? supporting : opposing).push("Model direction");
    }
    ...
    if (statements.length === 0) {
      return {
        ...
        result: null,
        confidence: 0,
        warnings: ["No evidence available to reason over"],
      };
    }
    ...
    const evidence: Evidence[] = statements.map((s) => ({
      key: s.kind,
      value: s.text,
      note: s.source,
    }));
