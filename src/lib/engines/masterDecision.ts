...
export function runMasterDecision(input: MasterDecisionInput): MasterDecision {
      ...
      pending(
        {
          id: "risk_gate",
          order: 15,
          label: "Risk Gate (authoritative)",
          layer: "risk",
          version: RISK_GATE_VERSION,
          responsibility: "Final veto over execution and position size; cannot be bypassed.",
        },
        reason,
      ),
    );
  ...
  if (!quality || !quality.usable) {
    return halted(
      "Pipeline halted: data quality gate failed",
      quality
        ? [
            "Data quality unacceptable",
            ...quality.issues.filter((i) => i.severity === "critical").map((i) => i.detail),
          ]
        : ["No market data available"],
    );
  }
  ...
  if (!analysis) {
    return halted("Pipeline halted: insufficient evidence", [
      "Insufficient evidence to construct a decision",
    ]);
  }
  ...
  const liquidityAnalysis =
    liquidityRun.result?.analysis ?? analysis.institutional?.liquidity ?? analyzeLiquidity(candles);

  // ---- Engine 2: Market Context -----------------------------------------
  const context = marketContextEngine(
    contextId,
    candles,
    timeframe,
    liquidityAnalysis,
  ...
  const contradictions = contradictionEngine(
    contextId,
    side,
  ...
  // ---- Engine 14: Portfolio -------------------------------------------
  const portfolioRun = portfolioEngine(
    contextId,
    symbol,
    side,
    confidence,
  ...
  if (side === "neutral") {
    ...
    reasons.push(
      `${confluenceTotal !== null ? `Confluence ${confluenceTotal.toFixed(0)}, ` : ""}confidence ${(confidence * 100).toFixed(0)}%`,
    );
  } else if (cReport.blocking) {
    action = "NO_TRADE";
    reasons.push(
      ...cReport.contradictions.filter((c) => c.severity === "high").map((c) => c.detail),
    );
  } else {
    action = "WATCH";
  ...
  pipeline.push({
    descriptor: {
      ...
      label: "Risk Gate (authoritative)",
      layer: "risk",
      version: riskGate.engine_version,
      responsibility: "Final veto over execution and position size; cannot be bypassed.",
    },
    result: riskGate,
    verdict: gate
      ? `${gate.approved ? "Approved" : "Not approved"} · ${gate.reason}`
      : "Risk gate did not run",
    skipped: false,
  });
  pipeline.push({
  ...
  const explain = explainabilityEngine(contextId, {
    ...
    rejectedAlternatives: (strategy.result?.candidates ?? [])
      .filter((c) => c.rejected)
      .map((c) => `${c.name}: ${c.rejectionReason ?? "rejected"}`),
    invalidationConditions:
      structure.result?.invalidationLevel != null
  ...
  const research = runResearch
    ? researchEngine(contextId, candles, symbol, timeframe, [
        strategy.result?.selected.label ?? analysis.selectedStrategy.label,
      ])
    : null;
  if (research) {
    push(
