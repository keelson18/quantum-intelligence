...
function classify(t: ClosedTradeInput, win: boolean): FailureClass {
  ...
  if (t.exitReason === "manual" && (t.takeProfit != null || t.stopLoss != null))
    return "Poor Execution";
  if (t.modelAgreed === false) return "Model Failure";
  if (
    t.regime &&
    t.strategy &&
    ["range_bound", "low_volatility"].includes(t.regime) &&
    /breakout|trend|momentum/i.test(t.strategy)
  )
...
export function tradeReviewEngine(
  contextId: string,
  trades: ClosedTradeInput[],
): EngineResult<TradeReviewResult> {
  return runEngine<TradeReviewResult>(D.id, D.version, contextId, () => {
    ...
    const reviews: TradeReview[] = trades.map((t) => {
      ...
      const rMultiple =
        risk && risk > 0 ? (t.side === "long" ? t.exit - t.entry : t.entry - t.exit) / risk : null;
      const failureClass = classify(t, win);
      const lessons: string[] = [];
      if (failureClass === "Poor Execution")
        lessons.push("Respect the planned exit levels instead of discretionary closes");
      if (failureClass === "Risk Failure")
        lessons.push("Never open a position without a defined invalidation level");
      if (failureClass === "Strategy Mismatch")
        lessons.push(`${t.strategy} is a poor fit for a ${t.regime} regime`);
      if (failureClass === "Data Failure")
        lessons.push("Block execution when data quality is below the floor");
      if (rMultiple != null && rMultiple > 0 && rMultiple < 1)
        lessons.push("Winner closed below 1R — let planned targets work");
      ...
      return {
        ...
        riskAssessment:
          risk != null
            ? `Planned risk ${risk.toFixed(2)} per unit, realised ${rMultiple?.toFixed(2) ?? "n/a"}R.`
            : "No stop level recorded — risk was undefined.",
        lessons,
        rMultiple,
      };
    });
    ...
    const evidence: Evidence[] = [
      ...
      ...Object.entries(
        reviews.reduce<Record<string, number>>(
          (acc, r) => ({ ...acc, [r.failureClass]: (acc[r.failureClass] ?? 0) + 1 }),
          {},
        ),
    ...
    return {
      ...
      evidence,
      warnings:
        reviews.length < 20 ? ["Fewer than 20 reviewed trades — conclusions are provisional"] : [],
    };
  });
}
