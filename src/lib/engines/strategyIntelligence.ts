  ...
  return runEngine<StrategyResultBundle>(D.id, D.version, contextId, () => {
    if (candles.length < 60) {
      return {
        ...
        result: null,
        confidence: 0,
        warnings: ["Insufficient history to evaluate strategies"],
      };
    }
    ...
    const candidates: StrategyCandidate[] = allSignals
      .map((s) => {
        const key = s.category || s.strategy.toLowerCase().replace(/\s+/g, "_");
        const clash = unsuitable.find(
          (u) => key.includes(u) || s.strategy.toLowerCase().includes(u),
        );
    ...
    const evidence: Evidence[] = [
      {
        ...
        note: `v${STRATEGY_VERSIONS[selected.name] ?? "1.0.0"} — ${selected.reason}`,
      },
      ...
      ...accepted
        .slice(0, 5)
        .map((c) => ({ key: c.name, value: Number(c.score.toFixed(3)), note: c.reason })),
    ];
    ...
    return {
      ...
      confidence:
        accepted.length === 0
          ? 0
          : selected.confidence * (regime ? Math.max(0.4, regime.stability) : 0.7),
      evidence,
      warnings:
        accepted.length === 0
          ? ["Every strategy candidate was rejected for the current regime"]
          : [],
    };
  });
}
