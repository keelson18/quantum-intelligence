  ...
  return runEngine<IndicatorResult>(D.id, D.version, contextId, () => {
    if (candles.length < 60) {
      return {
        ...
        result: null,
        confidence: 0,
        warnings: ["Insufficient history for indicator computation"],
      };
    }
    ...
    const push = (
      name: string,
      value: number,
      group: IndicatorReading["group"],
      bias: IndicatorReading["bias"],
      note: string,
    ) => {
      if (Number.isFinite(value))
        readings.push({ name, value: Number(value.toFixed(4)), group, bias, note });
    };
    ...
    push(
      ...
      "trend",
      "neutral",
      adxVal > 25 ? "Trending market" : "Weak/absent trend",
    );
    push(
      "ATR(14)",
      ind.atr[i],
      "volatility",
      "neutral",
      "Volatility unit used for stops and sizing",
    );
    ...
    push(
      ...
      ema50,
      "trend",
      close > ema50 ? "bullish" : "bearish",
      "Price vs intermediate trend",
    );
    push(
      "BB width",
      ind.bollinger.width[i],
      "volatility",
      "neutral",
    ...
    push(
      ...
      ind.obv[i],
      "volume",
      ind.obv[i] > (ind.obv[i - 5] ?? ind.obv[i]) ? "bullish" : "bearish",
    ...
    if (regime) {
      if (rsi > 70)
        regimeInterpretations.push(interpretIndicatorByRegime("rsi_overbought", regime).reason);
      if (rsi < 30)
        regimeInterpretations.push(interpretIndicatorByRegime("rsi_oversold", regime).reason);
      if (hist > 0)
        regimeInterpretations.push(interpretIndicatorByRegime("macd_bullish", regime).reason);
      if (hist < 0)
        regimeInterpretations.push(interpretIndicatorByRegime("macd_bearish", regime).reason);
    }
    ...
    const evidence: Evidence[] = readings.map((r) => ({
      key: r.name,
      value: r.value,
      weight: 1 / Math.max(1, readings.length),
      note: `${r.bias} — ${r.note}`,
    }));
