...
export function assessDataQuality(
  ...
  for (let i = 0; i < candles.length; i++) {
    ...
    if (c.volume === 0) {
      currentZeroRun++;
      zeroVolumeRun = Math.max(zeroVolumeRun, currentZeroRun);
  ...
  if (invalidOhlc)
    issues.push({
      ...
      severity: "critical",
      count: invalidOhlc,
      detail: `${invalidOhlc} bars violate OHLC relationships`,
    });
  if (negativeVolume)
    issues.push({
      code: "negative_volume",
      severity: "critical",
      count: negativeVolume,
      detail: `${negativeVolume} bars have invalid volume`,
    });
  if (duplicates)
    issues.push({
      ...
      severity: "warning",
      count: duplicates,
      detail: `${duplicates} duplicate timestamps`,
    });
  if (nonMonotonic)
    issues.push({
      ...
      severity: "critical",
      count: nonMonotonic,
      detail: `${nonMonotonic} out-of-order timestamps`,
    });
  if (gaps) {
    issues.push({
  ...
  if (zeroVolumeRun >= 5)
    issues.push({
      ...
      severity: "warning",
      count: zeroVolumeRun,
      detail: `${zeroVolumeRun} consecutive zero-volume bars`,
    });
...
export function dataQualityEngine(
  ...
  return runEngine("data_quality", DATA_QUALITY_VERSION, contextId, () => {
    ...
    const status = report.bars === 0 ? "insufficient_data" : report.usable ? "ok" : "degraded";
    return {
      status,
      result: report,
