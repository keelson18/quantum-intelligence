  ...
  return runEngine<KnowledgeResult>(D.id, D.version, contextId, () => {
    if (candles.length < 60) {
      return {
        ...
        result: null,
        confidence: 0,
        warnings: ["Insufficient history to derive institutional memory"],
      };
    }
    ...
    for (const r of memory.rejections.slice(0, 4)) {
      add(
        "validated_observation",
        `Price has rejected ${r.level.toFixed(2)} ${r.count} time(s)`,
        "market_memory.rejections",
        r.count >= 2,
        r.count,
      );
    }
    for (const f of memory.failedBreakouts.slice(0, 3)) {
      add(
        "behaviour",
        `Failed ${f.direction === "up" ? "upside" : "downside"} breakout at ${f.level.toFixed(2)} (${f.count}x)`,
        "market_memory.failedBreakouts",
        f.count >= 2,
        f.count,
      );
    }
    if (memory.historicalSupport.length) {
      add(
        "validated_observation",
        `Historical support cluster: ${memory.historicalSupport
          .slice(0, 3)
          .map((v) => v.toFixed(2))
          .join(", ")}`,
        "market_memory.historicalSupport",
        true,
        memory.historicalSupport.length,
      );
    }
    if (memory.historicalResistance.length) {
      add(
        "validated_observation",
        `Historical resistance cluster: ${memory.historicalResistance
          .slice(0, 3)
          .map((v) => v.toFixed(2))
          .join(", ")}`,
        "market_memory.historicalResistance",
        true,
        memory.historicalResistance.length,
      );
    }
    if (similarity && similarity.sampleSize > 0) {
      const validated =
        similarity.sampleQuality === "strong" || similarity.sampleQuality === "moderate";
      add(
    ...
    if (memory.reactionScore > 0.6) {
      add(
        "lesson",
        "Current levels have produced strong reactions — expect responsive rather than trending behaviour",
        "market_memory.reactionScore",
        true,
        1,
      );
    }
    ...
    const evidence: Evidence[] = [
      ...
      ...items.slice(0, 6).map((i) => ({
        key: i.kind,
        value: i.statement,
        note: `${i.source} (${i.validated ? "validated" : "unvalidated"})`,
      })),
    ];
    ...
    return {
      ...
      confidence:
        items.length === 0
          ? 0
          : Math.min(1, validatedCount / Math.max(3, items.length)) * memory.reactionScore,
      evidence,
