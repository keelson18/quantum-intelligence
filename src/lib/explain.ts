...
export function explainTrade(
  ...
  if (!isNaN(ind.bollinger.width[i]))
    indicators.push(`BB width=${ind.bollinger.width[i]?.toFixed(4)}`);
  ...
  if (rec.contributors.length > 0) {
    whyParts.push(
      `Top contributors: ${rec.contributors
        .slice(0, 5)
        .map((c) => `${c.source} (${c.side})`)
        .join(", ")}.`,
    );
  } else {
  ...
  if (ml) {
    whyParts.push(
      `ML model agrees: ${ml.prediction} with ${(ml.probability * 100).toFixed(0)}% probability.`,
    );
  }
  if (institutional) {
    whyParts.push(
      `Institutional grade: ${institutional.tradeGrade} (${institutional.confluence.total}/100 confluence, ${institutional.riskRating} risk).`,
    );
  ...
  if (rec.side !== "buy")
    alternatives.push(
      `If price breaks above ${(close * 1.02).toFixed(2)} with volume, a long entry would be triggered by the breakout strategy.`,
    );
  if (rec.side !== "sell")
    alternatives.push(
      `If price breaks below ${(close * 0.98).toFixed(2)}, the mean-reversion strategy may flag an oversold bounce.`,
    );
  if (rec.regime === "range")
    alternatives.push(
      "In a ranging market, mean reversion is preferred; a regime shift to expansion would switch to breakout.",
    );
  if (ml && ml.prediction !== (rec.side === "buy" ? "up" : rec.side === "sell" ? "down" : "flat")) {
    alternatives.push(
      `ML model diverges (${ml.prediction}). Monitor for confirmation before committing full position size.`,
    );
  }
  alternatives.push(
    "If no confirmation within the next 3 candles, the signal weakens and position size should be reduced.",
  );
  ...
  return {
    ...
    confidence:
      rec.contributors.reduce((sum, c) => sum + c.confidence * c.weight, 0) /
      (rec.contributors.reduce((s, c) => s + c.weight, 0) || 1),
    riskAssessment,
    stopLossReason,
    takeProfitReason,
    alternatives,
