...
describe("execution simulation", () => {
  it("applies adverse slippage to market entries", () => {
    const long = simulateFill(
      { side: "long", quantity: 2, requestedPrice: 100, orderType: "market" },
      A,
    );
    expect(long.fillPrice).toBeCloseTo(100.05);
    expect(long.fees).toBeCloseTo(100.05 * 2 * 0.001);

    const short = simulateFill(
      { side: "short", quantity: 2, requestedPrice: 100, orderType: "market" },
      A,
    );
  ...
  it("fills limit orders at the stated price but still charges fees", () => {
    const fill = simulateFill(
      { side: "long", quantity: 1, requestedPrice: 100, orderType: "limit" },
      A,
    );
  ...
  it("inverts slippage direction on exits", () => {
    const exit = simulateExitFill(
      { side: "long", quantity: 1, requestedPrice: 100, orderType: "market" },
      A,
    );
