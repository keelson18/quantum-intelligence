...
function hammer(candles: Candle[]): PatternHit | null {
  ...
  if (lw >= 2 * b && uw <= b * 0.3 && b / r < 0.35) {
    return {
      ...
      time: c.time,
      reason: "Hammer: long lower wick signals rejection of lower prices",
      overlays: [marker(c.time, "belowBar", "#22c55e", "arrowUp", "Hammer")],
    };
  }
  return null;
}
...
function invertedHammer(candles: Candle[]): PatternHit | null {
  ...
  if (uw >= 2 * b && lw <= b * 0.3 && b / r < 0.35) {
    return {
      ...
      index: candles.length - 1,
      time: c.time,
      reason: "Inverted hammer: potential bottom reversal",
      overlays: [marker(c.time, "belowBar", "#22c55e", "arrowUp", "Inv Hammer")],
    };
  }
  return null;
}
...
function shootingStar(candles: Candle[]): PatternHit | null {
  ...
  if (uw >= 2 * b && lw <= b * 0.3 && b / r < 0.35 && isBear(c)) {
    return {
      ...
      index: candles.length - 1,
      time: c.time,
      reason: "Shooting star: long upper wick signals rejection of highs",
      overlays: [marker(c.time, "aboveBar", "#ef4444", "arrowDown", "Star")],
    };
  }
  return null;
}
...
function hangingMan(candles: Candle[]): PatternHit | null {
  ...
  if (lw >= 2 * b && uw <= b * 0.3 && b / r < 0.35 && isBear(c)) {
    return {
      ...
      index: candles.length - 1,
      time: c.time,
      reason: "Hanging man: bearish reversal after uptrend",
      overlays: [marker(c.time, "aboveBar", "#ef4444", "arrowDown", "Hang Man")],
    };
  }
  return null;
}
...
function bullishEngulfing(candles: Candle[]): PatternHit | null {
  ...
  if (
    ...
    return {
      ...
      index: candles.length - 1,
      time: cur.time,
      reason: "Bullish engulfing: buyer overwhelms prior bearish candle",
      overlays: [marker(cur.time, "belowBar", "#22c55e", "arrowUp", "Bull Eng")],
    };
  }
  return null;
}
...
function bearishEngulfing(candles: Candle[]): PatternHit | null {
  ...
  if (
    ...
    return {
      ...
      index: candles.length - 1,
      time: cur.time,
      reason: "Bearish engulfing: seller overwhelms prior bullish candle",
      overlays: [marker(cur.time, "aboveBar", "#ef4444", "arrowDown", "Bear Eng")],
    };
  }
  return null;
}
...
function morningStar(candles: Candle[]): PatternHit | null {
  ...
  if (isBear(a) && body(b) < body(a) * 0.5 && isBull(c) && c.close > (a.open + a.close) / 2) {
    return {
      ...
      index: candles.length - 1,
      time: c.time,
      reason: "Morning star: three-candle bullish reversal",
      overlays: [marker(c.time, "belowBar", "#22c55e", "arrowUp", "Morning")],
    };
  }
  return null;
}
...
function eveningStar(candles: Candle[]): PatternHit | null {
  ...
  if (isBull(a) && body(b) < body(a) * 0.5 && isBear(c) && c.close < (a.open + a.close) / 2) {
    return {
      ...
      index: candles.length - 1,
      time: c.time,
      reason: "Evening star: three-candle bearish reversal",
      overlays: [marker(c.time, "aboveBar", "#ef4444", "arrowDown", "Evening")],
    };
  }
  return null;
}
...
function doji(candles: Candle[]): PatternHit | null {
  ...
  if (b / r < 0.1 && r > 0) {
    ...
    return {
      ...
      index: candles.length - 1,
      time: c.time,
      reason: "Doji: indecision — open and close nearly equal",
      overlays: [marker(c.time, "aboveBar", "#9ca3af", "arrowDown", "Doji")],
    };
  }
  return null;
}
...
function dragonflyDoji(candles: Candle[]): PatternHit | null {
  ...
  if (b / r < 0.1 && lw > r * 0.6 && uw < r * 0.1) {
    return {
      ...
      index: candles.length - 1,
      time: c.time,
      reason: "Dragonfly doji: buyers rejected lows",
      overlays: [marker(c.time, "belowBar", "#22c55e", "arrowUp", "Dragon")],
    };
  }
  return null;
}
...
function gravestoneDoji(candles: Candle[]): PatternHit | null {
  ...
  if (b / r < 0.1 && uw > r * 0.6 && lw < r * 0.1) {
    return {
      ...
      index: candles.length - 1,
      time: c.time,
      reason: "Gravestone doji: sellers rejected highs",
      overlays: [marker(c.time, "aboveBar", "#ef4444", "arrowDown", "Grave")],
    };
  }
  return null;
}
...
function threeWhiteSoldiers(candles: Candle[]): PatternHit | null {
  ...
  if (three.every(isBull) && three[2].close > three[1].close && three[1].close > three[0].close) {
    return {
      ...
      index: candles.length - 1,
      time: three[2].time,
      reason: "Three white soldiers: strong bullish momentum",
      overlays: [marker(three[2].time, "belowBar", "#22c55e", "arrowUp", "3 White")],
    };
  }
  return null;
}
...
function threeBlackCrows(candles: Candle[]): PatternHit | null {
  ...
  if (three.every(isBear) && three[2].close < three[1].close && three[1].close < three[0].close) {
    return {
      ...
      index: candles.length - 1,
      time: three[2].time,
      reason: "Three black crows: strong bearish momentum",
      overlays: [marker(three[2].time, "aboveBar", "#ef4444", "arrowDown", "3 Black")],
    };
  }
  return null;
}
...
function tweezerTop(candles: Candle[]): PatternHit | null {
  ...
  if (isBull(a) && isBear(b) && Math.abs(a.high - b.high) / a.high < 0.003) {
    return {
      ...
      index: candles.length - 1,
      time: b.time,
      reason: "Tweezer top: matching highs signal rejection",
      overlays: [marker(b.time, "aboveBar", "#ef4444", "arrowDown", "TweezT")],
    };
  }
  return null;
}
...
function tweezerBottom(candles: Candle[]): PatternHit | null {
  ...
  if (isBear(a) && isBull(b) && Math.abs(a.low - b.low) / a.low < 0.003) {
    return {
      ...
      index: candles.length - 1,
      time: b.time,
      reason: "Tweezer bottom: matching lows signal rejection",
      overlays: [marker(b.time, "belowBar", "#22c55e", "arrowUp", "TweezB")],
    };
  }
  return null;
}
...
function insideBar(candles: Candle[]): PatternHit | null {
  ...
  if (b.high <= a.high && b.low >= a.low) {
    return {
      ...
      overlays: [marker(b.time, "aboveBar", "#9ca3af", "arrowDown", "Inside")],
    };
  }
  return null;
}
...
function outsideBar(candles: Candle[]): PatternHit | null {
  ...
  if (b.high > a.high && b.low < a.low) {
    ...
    return {
      ...
      index: candles.length - 1,
      time: b.time,
      reason: "Outside bar: volatility expansion",
      overlays: [
        marker(
          b.time,
          isBull(b) ? "belowBar" : "aboveBar",
          isBull(b) ? "#22c55e" : "#ef4444",
          isBull(b) ? "arrowUp" : "arrowDown",
          "Outside",
        ),
      ],
    };
...
function detectHeadShoulders(candles: Candle[]): PatternHit | null {
  ...
  const isRegular =
    b.value > a.value && b.value > c.value && Math.abs(a.value - c.value) / b.value < 0.08;
  const isInverse =
    b.value < a.value && b.value < c.value && Math.abs(a.value - c.value) / b.value < 0.08;
  ...
  if (isRegular && last < neckline) {
    return {
      ...
      index: b.index,
      time: b.time,
      reason: "Bearish H&S confirmed, neckline broken",
      overlays: [lineOverlay([a, b, c], "#ef4444", "H&S")],
    };
  }
  if (isInverse && last > neckline) {
    return {
      ...
      index: b.index,
      time: b.time,
      reason: "Bullish inverse H&S confirmed",
      overlays: [lineOverlay([a, b, c], "#22c55e", "iH&S")],
    };
  }
  return null;
}
    ...
    if (Math.abs(a.value - b.value) / a.value < 0.03) {
      ...
      if (last < trough)
        return {
          ...
          index: b.index,
          time: b.time,
          reason: "Double top confirmed by breakdown",
          overlays: [lineOverlay([a, b], "#ef4444", "DT")],
        };
    }
  }
  if (lows.length >= 2) {
    ...
    if (Math.abs(a.value - b.value) / a.value < 0.03) {
      ...
      if (last > peak)
        return {
          ...
          index: b.index,
          time: b.time,
          reason: "Double bottom confirmed by breakout",
          overlays: [lineOverlay([a, b], "#22c55e", "DB")],
        };
    }
  }
  return null;
...
function detectTripleTopBottom(candles: Candle[]): PatternHit | null {
  ...
  if (highs.length >= 3) {
    const [a, b, c] = highs.slice(-3);
    const spread =
      (Math.max(a.value, b.value, c.value) - Math.min(a.value, b.value, c.value)) / a.value;
    if (spread < 0.04) {
      const last = candles[candles.length - 1].close;
      ...
      if (last < floor)
        return {
          ...
          index: c.index,
          time: c.time,
          reason: "Triple top breakdown",
          overlays: [lineOverlay([a, b, c], "#ef4444", "TT")],
        };
    }
  }
  if (lows.length >= 3) {
    ...
    if (spread < 0.04) {
      ...
      if (last > ceil)
        return {
          ...
          index: c.index,
          time: c.time,
          reason: "Triple bottom breakout",
          overlays: [lineOverlay([a, b, c], "#22c55e", "TB")],
        };
    }
  }
  return null;
...
function detectContinuation(candles: Candle[]): PatternHit | null {
  ...
  if (Math.abs(slopeHigh) < slopeLow * 0.2 && slopeLow > 0) {
    return {
      ...
      overlays: [
        lineOverlay([hh[0], hh[1]], "#f59e0b", "resist"),
        lineOverlay([ll[0], ll[1]], "#22c55e", "support"),
      ],
    };
  }
  ...
  if (Math.abs(slopeLow) < Math.abs(slopeHigh) * 0.2 && slopeHigh < 0) {
    return {
      ...
      index: candles.length - 1,
      time: candles[candles.length - 1].time,
      reason: "Descending triangle: flat support + falling resistance",
      overlays: [
        lineOverlay([hh[0], hh[1]], "#ef4444", "resist"),
        lineOverlay([ll[0], ll[1]], "#f59e0b", "support"),
      ],
    };
  }
  ...
  if (Math.sign(slopeHigh) !== Math.sign(slopeLow) && Math.abs(slopeHigh - slopeLow) > 0) {
    ...
    return {
      ...
      index: candles.length - 1,
      time: candles[candles.length - 1].time,
      reason: "Symmetrical triangle — breakout direction pending",
      overlays: [
        lineOverlay([hh[0], hh[1]], "#f59e0b", "resist"),
        lineOverlay([ll[0], ll[1]], "#f59e0b", "support"),
      ],
  ...
  if (
    ...
    Math.abs(slopeHigh - slopeLow) / (Math.abs(slopeHigh) + 1e-9) < 0.3
  ) {
    const dir = slopeHigh > 0 ? "sell" : "buy"; // counter-trend flag
    return {
      kind: "chart",
      name: slopeHigh > 0 ? "Bear Flag" : "Bull Flag",
      side: dir as Side,
      confidence: 0.5,
      index: candles.length - 1,
      time: candles[candles.length - 1].time,
      reason: "Flag pattern — continuation likely after consolidation",
      overlays: [
        lineOverlay([hh[0], hh[1]], "#06b6d4", "upper"),
        lineOverlay([ll[0], ll[1]], "#06b6d4", "lower"),
      ],
    };
  }
  // Pennant: converging but shorter than triangle.
  if (
    Math.abs(slopeHigh) < Math.abs(slopeLow) * 0.5 ||
    Math.abs(slopeLow) < Math.abs(slopeHigh) * 0.5
  ) {
    const last = candles[candles.length - 1].close;
    const side: Side = last > (hh[0].value + ll[0].value) / 2 ? "buy" : "sell";
    return {
      ...
      index: candles.length - 1,
      time: candles[candles.length - 1].time,
      reason: "Pennant: converging consolidation",
      overlays: [
        lineOverlay([hh[0], hh[1]], "#a855f7", "resist"),
        lineOverlay([ll[0], ll[1]], "#a855f7", "support"),
      ],
    };
  }
  return null;
...
function detectWedge(candles: Candle[]): PatternHit | null {
  ...
  if (Math.sign(slopeHigh) === Math.sign(slopeLow) && Math.abs(slopeHigh) > Math.abs(slopeLow)) {
    return {
      ...
      index: candles.length - 1,
      time: candles[candles.length - 1].time,
      reason: "Rising wedge: bearish reversal pattern",
      overlays: [
        lineOverlay([hh[0], hh[1]], "#ef4444", "wedgeH"),
        lineOverlay([ll[0], ll[1]], "#ef4444", "wedgeL"),
      ],
    };
  }
  if (Math.sign(slopeHigh) === Math.sign(slopeLow) && Math.abs(slopeLow) > Math.abs(slopeHigh)) {
    return {
      ...
      index: candles.length - 1,
      time: candles[candles.length - 1].time,
      reason: "Falling wedge: bullish reversal pattern",
      overlays: [
        lineOverlay([hh[0], hh[1]], "#22c55e", "wedgeH"),
        lineOverlay([ll[0], ll[1]], "#22c55e", "wedgeL"),
      ],
    };
  }
  return null;
...
function detectRectangle(candles: Candle[]): PatternHit | null {
  ...
  if (flatHigh && flatLow) {
    ...
    return {
      ...
      index: candles.length - 1,
      time: candles[candles.length - 1].time,
      reason: "Rectangle range — breakout direction sets trend",
      overlays: [
        lineOverlay([hh[0], hh[1]], "#6b7280", "top"),
        lineOverlay([ll[0], ll[1]], "#6b7280", "bottom"),
      ],
    };
  }
  return null;
  ...
  if (last > peak * 0.95 && last < peak * 1.02 && trough < peak * 0.85) {
    ...
    if (pullbackLow > trough && pullbackLow < last) {
      return {
        ...
        index: candles.length - 1,
        time: candles[candles.length - 1].time,
        reason: "Cup & handle: bullish continuation after rounded base",
        overlays: [
          lineOverlay(
            [
              { time: recent[0].time, value: peak },
              { time: recent[30].time, value: trough },
              { time: recent[recent.length - 1].time, value: last },
            ],
            "#22c55e",
            "C&H",
          ),
