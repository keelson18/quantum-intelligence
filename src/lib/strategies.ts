    ...
    overlays: [
      {
        ...
        id: "ema20",
        points: closes
          .map((_, j) => ({ time: candles[j].time, value: ind.ema[20][j] }))
      ...
      },
      {
        ...
        id: "ema50",
        points: closes
          .map((_, j) => ({ time: candles[j].time, value: ind.ema[50][j] }))
          .filter((p) => !isNaN(p.value)),
        color: "#f59e0b",
        label: "EMA50",
      },
      {
        ...
        id: "ema200",
        points: closes
          .map((_, j) => ({ time: candles[j].time, value: ind.ema[200][j] }))
          .filter((p) => !isNaN(p.value)),
        color: "#6b7280",
        label: "EMA200",
      },
...
function statArb(ctx: AnalysisContext): Signal {
  ...
  if (z < -2) {
    side = "buy";
    confidence = 0.5;
    reason = `Price well below VWAP (z=${z.toFixed(1)}) — mean revert up`;
  } else if (z > 2) {
    side = "sell";
    confidence = 0.5;
    reason = `Price well above VWAP (z=${z.toFixed(1)}) — mean revert down`;
  }
...
function pairs(_ctx: AnalysisContext): Signal {
  return {
    ...
    side: "neutral",
    confidence: 0.3,
    reason: "Pairs trading requires a correlated instrument pair",
  };
}
...
function smartMoney(ctx: AnalysisContext): Signal {
  ...
  const bullOB = smc.orderBlocks.filter(
    (ob) => ob.side === "bullish" && close >= ob.low && close <= ob.high,
  );
  const bearOB = smc.orderBlocks.filter(
    (ob) => ob.side === "bearish" && close >= ob.low && close <= ob.high,
  );
  if (inDiscount && bullOB.length) {
    side = "buy";
    confidence = 0.58;
    reason = "Bullish order block in discount zone";
  } else if (inPremium && bearOB.length) {
    side = "sell";
    confidence = 0.58;
    reason = "Bearish order block in premium zone";
...
export function collectOverlays(
  ...
  if (ctx.fib) {
    for (const r of ctx.fib.retracements) {
      overlays.push({
        ...
        id: `fib-r-${r.level}`,
        price: r.price,
        color: r.level === 0.618 ? "#10a37f" : "#9ca3af",
        label: `Fib ${(r.level * 100).toFixed(1)}%`,
      });
    }
    for (const e of ctx.fib.extensions) {
      overlays.push({
        ...
        price: e.price,
        color: "#a855f7",
        label: `Ext ${(e.level * 100).toFixed(1)}%`,
      });
    }
  }
  ...
  for (const ob of ctx.smc.orderBlocks.slice(-3)) {
    overlays.push({
      ...
      from: ob.low,
      to: ob.high,
      color: ob.side === "bullish" ? "rgba(34,197,94,0.12)" : "rgba(239,68,68,0.12)",
      label: `${ob.side === "bullish" ? "Bull" : "Bear"} OB`,
    });
  }
  for (const fvg of ctx.smc.fairValueGaps.slice(-3)) {
    overlays.push({
      ...
      from: fvg.bottom,
      to: fvg.top,
      color: fvg.side === "bullish" ? "rgba(16,163,127,0.1)" : "rgba(220,38,38,0.1)",
      label: "FVG",
    });
  }
  // Risk lines.
