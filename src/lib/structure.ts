...
export function analyzeMarketStructure(candles: Candle[]): MarketStructure {
  ...
  for (const h of highs) {
    if (h === prevHigh) {
      labels.push({ index: h.index, label: "HH" });
      continue;
    }
  ...
  for (const l of lows) {
    if (l === prevLow) {
      labels.push({ index: l.index, label: "HL" });
      continue;
    }
  ...
  if (highs.length >= 2) {
    ...
    if (lastPrice > lastHigh.value) {
      ...
      events.push({
        ...
        reason: wasBearish
          ? "Bearish structure broken — character change to bullish"
          : "Bullish break of structure — continuation higher",
      });
    }
  }
  if (lows.length >= 2) {
    ...
    if (lastPrice < lastLow.value) {
      ...
      events.push({
        ...
        level: lastLow.value,
        reason: wasBullish
          ? "Bullish structure broken — character change to bearish"
          : "Bearish break of structure — continuation lower",
      });
    }
  }
  ...
  if (recentHighs.length >= 2 && recentLows.length >= 2) {
    const higherHighs =
      recentHighs.length >= 2 && recentHighs[recentHighs.length - 1].value > recentHighs[0].value;
    const higherLows =
      recentLows.length >= 2 && recentLows[recentLows.length - 1].value > recentLows[0].value;
    const lowerHighs =
      recentHighs.length >= 2 && recentHighs[recentHighs.length - 1].value < recentHighs[0].value;
    const lowerLows =
      recentLows.length >= 2 && recentLows[recentLows.length - 1].value < recentLows[0].value;
    if (higherHighs && higherLows) regime = "trend_up";
      ...
      Math.abs(recentHighs[recentHighs.length - 1].value - recentHighs[0].value) /
        recentHighs[0].value <
        0.03 &&
      Math.abs(recentLows[recentLows.length - 1].value - recentLows[0].value) /
        recentLows[0].value <
        0.03
    ) {
      ...
      Math.abs(
        recentHighs[recentHighs.length - 1].value - recentLows[recentLows.length - 1].value,
      ) /
        recentHighs[recentHighs.length - 1].value >
      0.08
    ) {
      regime = "expansion";
    }
...
export function analyzeSmartMoney(candles: Candle[]): SmartMoney {
  if (candles.length < 20) {
    return {
      ...
      equalHighs: [],
      equalLows: [],
      premiumDiscount: { premium: 0, discount: 0, midpoint: 0 },
      breakerBlocks: [],
    };
  }
  const { highs, lows } = findSwings(candles, 3, 3);
  ...
  const rangeHigh = highs.length
    ? Math.max(...highs.slice(-3).map((h) => h.value))
    : candles[candles.length - 1].high;
  const rangeLow = lows.length
    ? Math.min(...lows.slice(-3).map((l) => l.value))
  ...
  const breakerBlocks: OrderBlock[] = recentOBs
    .filter((ob) => {
      ...
      const breached = after.some((c) =>
        ob.side === "bullish" ? c.close < ob.low : c.close > ob.high,
      );
      return breached;
    })
    .slice(-3);
